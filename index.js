...
const { configurarRutasEstudiantes } = require('./microservices/academia/Estudiantes');
const { configurarRutasGestionAcademica } = require('./microservices/academia/GestionAcademico');
...
const { configurarRutasDashboardPagos } = require('./microservices/pagos/dashboardPagos');
...
const { configurarRutasIngresosAcademicos } = require('./microservices/pagos/ingresosAcademicos');
configurarRutasIngresosAcademicos(app, pool, authMiddleware);

// Configurar rutas de compromiso económico y pagos
const { configurarRutasCompromisoEconomico } = require('./microservices/pagos/compromisoEconomico');
...
app.get('/api/becas', authMiddleware, async (req, res) => {
  try {
    const [becas] = await pool.execute('SELECT * FROM becas ORDER BY descripcion');
    res.json(becas);
  } catch (error) {
  ...
  try {
    ...
    res.status(500).json({
      ...
      suggestions: {
        ...
        'ER_ACCESS_DENIED_ERROR': 'Verifica las credenciales de la base de datos',
...
async function verificarConexionBD() {
  ...
  console.log('');
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║        BACKEND PRINCIPAL - UNIDAD EDUCATIVA EMI              ║');
  console.log('╚══════════════════════════════════════════════════════════════╝');
  console.log('');
  ...
  for (let intento = 1; intento <= maxReintentos; intento++) {
    try {
      ...
      const [result] = await pool.query('SELECT 1 as test, NOW() as timestamp, DATABASE() as database_name, VERSION() as mysql_version');
      ...
      console.error(`❌ Error intentando conectar a la base de datos (intento ${intento}/${maxReintentos}):`, error.message);
...
async function iniciarServidor() {
  try {
    ...
    const { inicializar: inicializarWhatsApp } = require('./microservices/academia/whatsappServiceSingleton');
    ...
    const gracefulShutdown = async () => {
      try {
        const { obtenerInstancia } = require('./microservices/academia/whatsappServiceSingleton');
