...
function dividirTextoEnChunksSemanticos(texto, opciones = {}) {
  ...
  while ((match = patronTitulo.exec(texto)) !== null) {
    secciones.push({ tipo: 'titulo', texto: match[1].trim(), posicion: match.index });
  }

  // Buscar capítulos
  patronCapitulo.lastIndex = 0;
  while ((match = patronCapitulo.exec(texto)) !== null) {
    secciones.push({ tipo: 'capitulo', texto: match[1].trim(), posicion: match.index });
  }
  ...
  while ((match = patronArticuloConParen.exec(texto)) !== null) {
    if (!posicionesArticulos.has(match.index)) {
      ...
      secciones.push({ tipo: 'articulo', texto: match[1].trim(), posicion: match.index });
    }
  }
  ...
  if (secciones.length === 0) {
    ...
    const partes = texto.split(splitPorArticulo).filter(p => p.trim().length > 0);
    if (partes.length > 1) {
      partes.forEach((parte, idx) => {
        const contenido = parte.trim();
        if (contenido.length >= minChunkSize) {
          const matchArt = contenido.match(/Art[íi]culo\s*(?:N[°º]?\s*)?(\d+)[\.\-]?\s*([^\n]*)/i);
          chunks.push({
            texto: contenido,
            metadata: {
          ...
          chunks.push({ texto: contenido, metadata: { documento: nombreDocumento, tipo: tipoDocumento, titulo: '', capitulo: '', articulo: String(idx + 1), titulo_articulo: '', parte: null } });
        }
      });
      if (chunks.length > 0) {
        console.log(`📄 [ChunkingSemantico] Documento "${nombreDocumento}": ${chunks.length} chunks (split por Artículo)`);
        return chunks;
      }
    }
  ...
  for (let i = 0; i < secciones.length; i++) {
    ...
    const siguientePosicion = i < secciones.length - 1 ? secciones[i + 1].posicion : texto.length;
    ...
    const contenidoCompleto = texto.slice(seccion.posicion, siguientePosicion).trim();

    // Extraer número y título del artículo (paréntesis opcional)
    const matchNumero = seccion.texto.match(/Art[íi]culo\s*(?:N[°º]?\s*)?(\d+)/i);
    const matchTituloArt = seccion.texto.match(/\(([^)]+)\)/) || seccion.texto.match(/[\.\-]\s*([^\n(]+)/);
    ...
    if (contenidoCompleto.length > maxChunkSize) {
      const subChunks = dividirTextoEnChunks(contenidoCompleto, maxChunkSize, 150);
      subChunks.forEach((subChunk, idx) => {
        chunks.push({
          ...
          metadata: {
            ...
            parte: subChunks.length > 1 ? `${idx + 1}/${subChunks.length}` : null
          }
        });
      });
      ...
      introChunks.forEach((chunk, idx) => {
        chunks.unshift({
          ...
          metadata: {
            ...
            articulo: '',
            titulo_articulo: '',
            parte: introChunks.length > 1 ? `${idx + 1}/${introChunks.length}` : null
          }
        });
      });
    }
  ...
  console.log(`📄 [ChunkingSemantico] Documento "${nombreDocumento}": ${chunks.length} chunks creados`);
  return chunks;
}
...
async function cargarModeloEmbeddings() {
  ...
  try {
    if (!pipeline) {
      console.log('🔍 [Agente] Intentando carga dinámica de @xenova/transformers...');
      try {
        ...
        console.error('❌ [Agente] Error en import() dinámico:', importError.message);
        return null;
      }
    }
    ...
    if (pipeline) {
      ...
      modeloEmbeddings = await pipeline('feature-extraction', 'Xenova/multilingual-e5-base');
    ...
    try {
      if (pipeline) {
        modeloEmbeddings = await pipeline('feature-extraction', 'Xenova/all-MiniLM-L6-v2');
        console.log('✅ Modelo de embeddings base cargado (fallback)');
        return modeloEmbeddings;
      }
    } catch (fallbackError) {
      console.error('❌ Error crítico en fallback de embeddings:', fallbackError.message);
...
async function cargarDocumentosDesdeBD() {
  try {
    ...
    if (documentosBdCache && ultimaCargaDocumentos &&
    ...
    if (documentos && documentos.length > 0) {
      console.log(`📚 [CargarDocumentos] Procesando ${documentos.length} documento(s) con chunking semántico...`);

      for (const doc of documentos) {
        if (!doc.texto_extraido || doc.texto_extraido.trim().length === 0) {
          console.warn(`⚠️ Documento "${doc.nombre}" no tiene texto extraído, saltando...`);
          continue;
        }

        // Usar chunking semántico para cada documento
        const chunksConMetadata = dividirTextoEnChunksSemanticos(doc.texto_extraido, {
          maxChunkSize: 2800,
          minChunkSize: 50,
        ...
        if (Array.isArray(chunksConMetadata) && chunksConMetadata.length > 0) {
          ...
          if (typeof chunksConMetadata[0] === 'object' && chunksConMetadata[0].texto) {
    ...
    try {
      if (fs.existsSync(RUTA_REGLAMENTO)) {
        ...
        const chunksReglamento = dividirTextoEnChunksSemanticos(contenidoReglamento, {
          maxChunkSize: 1500,
          minChunkSize: 200,
        ...
        if (Array.isArray(chunksReglamento) && chunksReglamento.length > 0) {
          if (typeof chunksReglamento[0] === 'object' && chunksReglamento[0].texto) {
      ...
      console.log('ℹ️  No se encontró reglamento estático, usando solo documentos de BD');
    }

    console.log(`✅ [CargarDocumentos] Total: ${todosLosChunks.length} chunks con metadata cargados`);
    ...
    try {
      if (fs.existsSync(RUTA_REGLAMENTO)) {
        ...
        return { chunks, metadata: chunks.map(() => ({ documento: 'fallback', tipo: 'reglamento' })) };
      }
    } catch (fallbackError) {
  ...
  try {
    ...
    if (cache && cache.chunks && cache.chunks.length > 0) {
      ...
      if (cache.embeddings && cache.embeddings.length > 0) {
        ...
        console.log(`✅ [CargarEmbeddings] Cargados ${docsReglamento.length} chunks y ${docsEmbeddings.length} embeddings desde cache BD`);
        return; // Todo listo desde cache
      } else {
        console.log(`📦 [CargarEmbeddings] Cargados ${docsReglamento.length} chunks desde cache, pero sin embeddings`);
      }
    }

    // 2. Si no hay cache, cargar documentos desde BD
    if (!docsReglamento || docsReglamento.length === 0) {
      console.log('📚 [CargarEmbeddings] No hay cache, cargando documentos desde BD...');
    ...
    console.log(`📊 [CargarEmbeddings] Calculando embeddings para ${docsReglamento.length} chunks...`);
    ...
    const modelo = await Promise.race([
      ...
      new Promise((resolve) => setTimeout(() => resolve(null), EMBEDDINGS_TIMEOUT_MS))
    ]);
    if (!modelo) {
      console.warn(`⚠️ [CargarEmbeddings] Timeout (${EMBEDDINGS_TIMEOUT_MS}ms) al cargar modelo. Continuando sin embeddings.`);
    }
    if (modelo) {
      const embeddings = [];
      ...
      for (let i = 0; i < docsReglamento.length; i++) {
        ...
        const output = await modelo(texto, { pooling: 'mean', normalize: true });
        embeddings.push(Array.from(output.data));

        // Log de progreso cada 50 chunks
        if ((i + 1) % 50 === 0) {
          const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
          console.log(`  📈 Progreso: ${i + 1}/${docsReglamento.length} chunks (${elapsed}s)`);
        }
      }
      docsEmbeddings = embeddings;

      const totalTime = ((Date.now() - startTime) / 1000).toFixed(1);
      console.log(`✅ [CargarEmbeddings] ${embeddings.length} embeddings calculados en ${totalTime}s`);

      // 4. Guardar embeddings en cache BD para próxima vez
      // (Esto se hará cuando se suba un documento, no aquí para no duplicar)
    } else {
      console.warn('⚠️  Modelo de embeddings no disponible, usando búsqueda por keywords');
    }
  } catch (error) {
 ...
 */
async function procesarYGuardarChunksEmbeddings(documentoId, textoExtraido, nombre, tipo) {
  if (!textoExtraido || textoExtraido.trim().length === 0) {
    console.warn(`⚠️ Documento ID ${documentoId} sin texto, omitiendo chunks/embeddings`);
  ...
  if (!modelo) {
    console.warn('⚠️ Modelo de embeddings no disponible, chunks guardados sin embeddings');
    return { chunks: chunksConMetadata.length, embeddings: 0 };
  }

  const textos = chunksConMetadata.map((c) => (typeof c === 'object' && c.texto ? c.texto : c));
  const embeddings = [];
  for (let i = 0; i < textos.length; i++) {
    const output = await modelo(textos[i], { pooling: 'mean', normalize: true });
    embeddings.push(Array.from(output.data));
  }

  const okEmb = await guardarEmbeddingsChunks(documentoId, embeddings);
  return { chunks: chunksConMetadata.length, embeddings: okEmb ? embeddings.length : 0 };
}
...
async function recuperarContextoDesdeReglamento(pregunta, k = 8) {
  ...
  if (!documentosBdCache || !ultimaCargaDocumentos ||
    ...
    try {
      ...
      console.warn('⚠️  Error al recargar documentos, usando cache:', error.message);
    }
  }
  ...
  const matchArticulo = preguntaLower.match(/art[íi]culo\s*(?:n[°º]?\s*)?(\d+)/i);
  if (matchArticulo) {
    ...
    for (const [ordinal, num] of Object.entries(ordinales)) {
      const pat = new RegExp(`(?:^|[^a-z])${ordinal.replace(/[áéíóú]/g, '.')}\\s+art[íi]culo|art[íi]culo\\s+${ordinal.replace(/[áéíóú]/g, '.')}|cu[áa]l\\s+es\\s+el\\s+${ordinal.replace(/[áéíóú]/g, '.')}\\s+art[íi]culo`, 'i');
  ...
  const esPreguntaSobreFechas = /cu[áa]ndo|fecha|inicio|empiezan|comienzan|apertura|calendario/i.test(preguntaLower);
  const esPreguntaSobreRequisitos = /requisito|documento|necesito|inscrib|inscripci[óo]n/i.test(preguntaLower);
  const esPreguntaSobreReglas = /regla|norma|prohib|falta|sanc/i.test(preguntaLower);
  const esPreguntaSobreBecas = /beca|descuento|beneficio/i.test(preguntaLower);
  const esPreguntaListaAmplia = /cu[áa]les?\s+son|qu[eé]\s+(becas?|reglamentos|tipos)|(becas?|descuentos?)\s+existen|dedicad[oa]s?\s+(a|para)|dirigid[oa]s?\s+(a|para)|para\s+(padres|estudiantes|madres|apoderados|tutores)/i.test(preguntaLower);
  const esPreguntaPrimerosArticulos = /primeros?\s+art[íi]culos?|art[íi]culos?\s+primeros?/i.test(preguntaLower);
  const esPreguntaSobrePadresOEstudiantes = /padre|madre|estudiante|apoderado|tutor|padres\b|madres\b|estudiantes\b/i.test(preguntaLower);
  ...
  if (esPreguntaPrimerosArticulos || esPreguntaListaAmplia) kAjustado = Math.max(kAjustado, 18);
  ...
  const sinonimos = {
    'inicio': ['inicio', 'comienzo', 'iniciación', 'apertura', 'empieza', 'empiezan', 'comienza', 'comenzar', 'empezar', 'inicia'],
    'clases': ['clases', 'año escolar', 'periodo académico', 'ciclo escolar', 'gestion', 'gestión', 'actividades'],
    'fecha': ['fecha', 'fechas', 'día', 'días', 'calendario', 'cuando', 'cuándo'],
    'inscripcion': ['inscripción', 'inscripciones', 'inscribir', 'matricula', 'matrícula'],
    'requisitos': ['requisitos', 'documentos', 'necesario', 'requerir', 'presentar'],
    'beca': ['beca', 'becas', 'descuento', 'beneficio', 'ayuda'],
    'regla': ['regla', 'reglas', 'norma', 'normas', 'prohibido', 'sanción', 'falta'],
    'padre': ['padre', 'padres', 'madre', 'madres', 'apoderado', 'apoderados', 'tutor', 'tutores', 'familia', 'familiar'],
    'estudiante': ['estudiante', 'estudiantes', 'alumno', 'alumnos', 'educando', 'educandos']
  };
  ...
  const scores = docsReglamento.map((doc, idx) => {
    ...
    if (esPreguntaSobreBecas) {
      if (metadata.tipo === 'becas' || metadata.titulo_articulo?.toLowerCase().includes('beca') || metadata.titulo_articulo?.toLowerCase().includes('descuento')) score += 20;
      ...
      if (/beca|descuento|art[íi]culo\s*\d+|hermanos|fidelidad/i.test(docLower)) score += 12;
    }
    ...
    if (esPreguntaSobrePadresOEstudiantes) {
      if (/padre|madre|apoderado|tutor|familia|familiar/i.test(docLower)) score += 12;
      if (/estudiante|alumno|educando/i.test(docLower)) score += 12;
    }

    // 2d. BOOST para preguntas amplias (cuáles son, reglamentos para X)
    if (esPreguntaListaAmplia && (metadata.articulo || metadata.capitulo)) score += 8;
  ...
  if (esPreguntaSobreBecas) {
    const idxDocBecas = scores
      .filter(s => /beca|descuento/.test((s.metadata?.documento || '').toLowerCase()))
      .map(s => s.idx);
    const idxResto = scores.filter(s => !idxDocBecas.includes(s.idx)).map(s => s.idx);
...
async function obtenerEsquemaBd(pool) {
  ...
  try {
    ...
    const descripcionesTablas = {
      'estudiantes': 'Tabla principal de estudiantes. Contiene: nombre, apellido_paterno, apellido_materno, ci_estudiante, fecha_nacimiento, datos de padres, etc.',
      'inscripciones': 'Inscripciones de estudiantes a niveles/cursos. Relaciona estudiantes con niveles, cursos, bloques y becas.',
      'compromiso_economico': 'Compromisos económicos de estudiantes. Relaciona estudiantes con inscripciones y becas. Contiene total_cuotas, total_material, total_general.',
      'pagos_mensuales': 'Pagos mensuales por compromiso. Cada fila es un mes (febrero=2, marzo=3, etc.). Contiene: mes (1-12), anio, nombre_mes, monto_esperado, monto_pagado, estado (pendiente/parcial/pagado/vencido).',
      'pagos_realizados': 'Registro de pagos realizados. Relaciona con compromiso_economico. Contiene: fecha_pago, monto, tipo_pago (cuota/material/ambos), mes, anio.',
      'nivel': 'Niveles académicos (ej: PRIMER NIVEL, SEGUNDO NIVEL). Tiene precio y meses (JSON array).',
      'curso': 'Cursos dentro de niveles.',
      'bloque': 'Bloques académicos.',
      'becas': 'Becas disponibles con descuento.',
      'servicios_estudiante': 'Servicios adicionales contratados por estudiantes (ej: APOYO ESCOLAR).',
      'ingresos': 'Registro de todos los ingresos (académicos y servicios).',
      'usuarios': 'Usuarios del sistema con roles.',
      'roles': 'Roles del sistema (Administrador, Director, Secretaria, Cajero, etc.).'
    };
          ...
          try {
            const continuation = await model.generateContent({
              contents: [
                ...
                {
                  role: 'user',
                  parts: [{ text: 'Continúa la respuesta anterior desde donde te quedaste. Solo continúa, no repitas.' }]
                }
              ],
              generationConfig: {
...
async function buscarHijosPorDatosPadre(pool, datosPadre) {
  try {
    ...
    if (datosPadre.nombre_padre && datosPadre.apellido_padre) {
      ...
      parametros.push(`%${datosPadre.nombre_padre}%`, `%${datosPadre.apellido_padre}%`);
    }
    ...
    if (datosPadre.nombre_madre && datosPadre.apellido_madre) {
      ...
      parametros.push(`%${datosPadre.nombre_madre}%`, `%${datosPadre.apellido_madre}%`);
    }

    if (condiciones.length === 0) {
      console.log('⚠️ [buscarHijosPorDatosPadre] No hay criterios suficientes para buscar');
      return [];
    }
    ...
    console.log(`✅ [buscarHijosPorDatosPadre] Encontrados ${estudiantes.length} estudiante(s) relacionado(s)`);
    estudiantes.forEach(est => {
      console.log(`  - ID: ${est.id}, Nombre: ${est.nombre} ${est.apellido_paterno || ''} (Nivel: ${est.nivel_nombre || 'Sin nivel'})`);
    });
...
class HerramientaBaseDatos extends Herramienta {
  constructor(pool) {
    super('base_datos', 'Consulta información de la base de datos (estudiantes, pagos, inscripciones, etc.)');
    this.pool = pool;
  }

  async ejecutar(pregunta, infoRemitente = null, contextoHistorial = '', infoUsuario = null) {
    try {
      ...
      if (infoUsuario) {
        ...
        if (rol === 'Cajero') {
          const consultasRestringidas = [
            'eliminar', 'borrar', 'modificar', 'cambiar', 'actualizar',
        ...
        if (rol === 'Secretaria') {
          const consultasRestringidas = [
            'eliminar', 'borrar', 'modificar usuarios', 'cambiar roles',
      ...
      const preguntaPorTodosLosHijos = /\bmis\s+hijos\b|\bmis\s+niños\b|\bde\s+mis\s+hijos\b|\bpara\s+mis\s+hijos\b|\btodos\s+mis\s+hijos\b|\bcuota[s]?\s+de\s+mis\s+hijos\b|\bcuanto\s+(tengo|debo|pagar).*mis\s+hijos\b|\bcuota\s+de\s+(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)\s+de\s+mis\s+hijos\b/i.test(preguntaLower);
      ...
      const palabrasExcluidas = ['cuando', 'cuanto', 'que', 'qué', 'cual', 'cuál', 'donde', 'dónde', 'como', 'cómo', 'porque', 'por qué', 'para', 'con', 'sin', 'sobre', 'bajo', 'entre', 'hasta', 'desde', 'febrero', 'marzo', 'abril', 'mayo', 'junio', 'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre', 'vence', 'vencimiento', 'cuota', 'mensualidad', 'pago'];
      const relacionesEstudiante = 'hija|hijo|estudiante|sobrino|sobrina|nieto|nieta|vecino|vecina|amigo|amiga|familiar|tutor|tutora';
      const relacionesNombre = `(?:${relacionesEstudiante})`;
      const patronesNombre = [
        new RegExp(`(?:mi|de mi)\\s+${relacionesNombre}\\s+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]{2,}(?:\\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñ]*)*)`, 'i'),
        new RegExp(`${relacionesNombre}\\s+([A-ZÁÉÍÓÚÑ][a-záéíóúñ]{2,}(?:\\s+[A-ZÁÉÍÓÚÑ][a-záéíóúñ]*)*)`, 'i'),
      ...
      const palabrasComunes = ['hija', 'hijo', 'estudiante', 'sobrino', 'sobrina', 'nieto', 'nieta', 'vecino', 'vecina', 'amigo', 'amiga', 'familiar', 'tutor', 'tutora', 'cuota', 'pago', 'mensualidad', 'deuda', 'me', 'puede', 'decir', 'febrero', 'marzo', 'abril', 'mayo', 'junio', 'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre'];
      ...
      for (const pat of patronesMisHijosVarios) {
        ...
        while ((m = pat.exec(pregunta)) !== null) {
          ...
          const partes = bloque.split(/\s*,\s*|\s+y\s+/).map(s => s.trim()).filter(Boolean);
          for (const n of partes) {
            const nombreLower = n.toLowerCase();
            if (n.length > 0 && !palabrasComunes.includes(nombreLower) && !palabrasExcluidas.includes(nombreLower) && !nombresHijosMencionados.includes(n)) {
      ...
      for (const patron of patronesNombre) {
        ...
        for (const match of matches) {
          if (match && match[1]) {
            ...
            if (!palabrasComunes.includes(nombreLower) && !palabrasExcluidas.includes(nombreLower) && nombreExtraido.length > 0) {
      ...
      if (infoRemitente) {
        try {
          ...
          const estudiantes = await buscarHijosPorDatosPadre(this.pool, datosBusqueda);
          ...
          if (estudiantes && estudiantes.length > 0) {
            ...
            if (nombresHijosMencionados.length > 0) {
              ...
              const estudiantesFiltrados = estudiantes.filter(est => {
                ...
                return nombresHijosMencionados.some(nombreMencionado => {
                  ...
                  if (nombreMencionadoLower.length === 1) {
                    return soloNombre.startsWith(nombreMencionadoLower) ||
                      nombreCompleto.includes(nombreMencionadoLower);
                  } else {
                    return nombreCompleto.includes(nombreMencionadoLower) ||
              ...
              if (estudiantesFiltrados.length > 0) {
                ...
                console.log(`✅ Filtrado: Encontrado ${estudiantesFiltrados.length} estudiante(s) que coinciden con los nombres: ${nombresHijosMencionados.join(', ')}`);
              } else {
                console.log(`⚠️ No se encontró estudiante con los nombres mencionados, usando todos los estudiantes relacionados`);
              ...
              const listaHijos = estudiantes.map((est, idx) => {
              ...
              console.log(`📊 [HerramientaBaseDatos] Padre con ${estudiantes.length} hijos detectado. Se preguntará sobre cuál.`);
            ...
            if (!contextoRemitente.includes('MÚLTIPLES HIJOS DETECTADOS')) {
              ...
              if (nombresHijosMencionados.length > 0 && estudiantesRelacionados.length < estudiantes.length) {
            ...
            console.log(`✅ [HerramientaBaseDatos] Estudiantes relacionados encontrados: ${estudiantesRelacionados.length}`);
            estudiantesRelacionados.forEach(est => {
              console.log(`  - ID: ${est.id}, Nombre: ${est.nombre} ${est.apellido_paterno || ''}`);
            });
          } else {
            console.log(`⚠️ [HerramientaBaseDatos] No se encontraron estudiantes relacionados para el remitente`);
          }
        } catch (error) {
          console.error('❌ [HerramientaBaseDatos] Error al buscar estudiantes relacionados:', error.message);
        }
      } else {
        console.log(`⚠️ [HerramientaBaseDatos] No hay infoRemitente disponible`);
        ...
        const esUsuarioAdmin =
          infoUsuario &&
          ['Administrador', 'Director', 'Secretaria', 'Cajero'].includes(infoUsuario.rol);

        if (!esUsuarioAdmin) {
          const mencionaPagos = /(cuánto|cuanto).*debo/i.test(pregunta) || /(pago|cuota|mensualidad|deuda)/i.test(pregunta);
          const esPrimeraPersona = /\bmi(s)?\b/i.test(pregunta) || /\byo\b/i.test(pregunta);
      ...
      const relacionesHorario = 'hijo|hija|estudiante|sobrino|sobrina|nieto|nieta|vecino|vecina|amigo|amiga|familiar|tutor|tutora';
      ...
      if (esPreguntaHorario && estudiantesRelacionados && estudiantesRelacionados.length > 0) {
        ...
        if (estudiantesRelacionados.length > 1 && (!nombresHijosMencionados || nombresHijosMencionados.length === 0)) {
          const listaHijos = estudiantesRelacionados.map((est, idx) => {
            const nivel = est.nivel_nombre || 'Sin nivel asignado';
            const nombreCompleto = `${est.nombre} ${est.apellido_paterno || ''} ${est.apellido_materno || ''}`.trim();
            return `${idx + 1}. *${nombreCompleto}* - ${nivel}`;
          }).join('\n');
        ...
        const [rowsHorario] = await this.pool.query(sqlHorario, [estudianteSeleccionado.id]);
        ...
        const horaEntrada = info.hora_entrada ? String(info.hora_entrada).slice(0, 5) : null;
        const horaSalida = info.hora_salida ? String(info.hora_salida).slice(0, 5) : null;
...
SQL:`;
      ...
      try {
        ...
        if (esErrorCuotaGemini(geminiError)) {
          console.warn('⚠️ Gemini sin cuota para generar SQL. Se usará SQL de respaldo.');
      ...
      // Limpiar SQL
      sqlGenerado = sqlGenerado.replace(/```sql/g, '').replace(/```/g, '').trim();
      ...
      if (sqlMatch) {
        ...
        console.log('⚠️ No se encontró SQL válido. Intentando SQL de respaldo...');
      ...
      if (!sqlGenerado || sqlGenerado.length < 10) {
        console.log('⚠️ SQL generado está vacío o es muy corto. Intentando SQL de respaldo...');
        sqlGenerado = null; // Marcar como inválido para que se genere el SQL de respaldo
      }

      if (sqlGenerado && !sqlGenerado.trim().toUpperCase().startsWith('SELECT')) {
        console.log('⚠️ SQL generado no comienza con SELECT. Intentando SQL de respaldo...');
        sqlGenerado = null; // Marcar como inválido para que se genere el SQL de respaldo
      }
      ...
      let sqlIncompleto = !sqlGenerado || !sqlUpper.includes('FROM') || sqlGenerado.trim().length < 50;
      // Si el padre preguntó por "mis hijos" y hay varios: el SQL DEBE traer id_estudiante para separar por hijo
      if (!sqlIncompleto && sqlGenerado && preguntaPorTodosLosHijos && estudiantesRelacionados.length > 1) {
        const mencionaPagosEnPregunta = /\b(cuota|pago|mensualidad|cuanto\s+debo|debo\s+del|debo\s+de)\b/i.test(pregunta);
        const sqlTraeIdEstudiante = /id_estudiante|e\.id\s+FROM|SELECT\s+.*\be\.id\b/i.test(sqlGenerado);
        if (mencionaPagosEnPregunta && !sqlTraeIdEstudiante) {
          console.log('🔄 SQL sin id_estudiante con múltiples hijos: forzando SQL de respaldo para separar por estudiante.');
          sqlGenerado = null;
          sqlIncompleto = true;
        }
      }
      ...
      if (sqlIncompleto) {
        console.error('❌ SQL generado está incompleto o no incluye FROM:', sqlGenerado);
        console.log(`📋 Estudiantes relacionados: ${estudiantesRelacionados.length}`);
        ...
        const preguntaLower = pregunta.toLowerCase();
        const mencionaPagos = preguntaLower.includes('cuota') || preguntaLower.includes('mensualidad') ||
          preguntaLower.includes('pago') || preguntaLower.includes('vencimiento') ||
          preguntaLower.includes('cuanto debo') || preguntaLower.includes('cuánto debo') ||
          preguntaLower.includes('debo del mes') || preguntaLower.includes('debo de') ||
          preguntaLower.includes('cuanto debe') || preguntaLower.includes('cuánto debe');

        console.log(`🔍 Menciona pagos: ${mencionaPagos}`);
        console.log(`🔍 Estudiantes relacionados disponibles: ${estudiantesRelacionados.length > 0}`);

        if (estudiantesRelacionados.length > 0 && mencionaPagos) {
          console.log('🔄 Generando SQL de respaldo para pregunta sobre pagos...');

          // Normalizar errores ortográficos en nombres de meses (ej. "marsso" -> "marzo")
          const { preguntaNormalizada, mesCorregido } = normalizarMesesEnPregunta(pregunta);
          ...
          if (mesCorregidoPregunta) {
            console.log(`📝 Mes corregido en pregunta: "${mesCorregidoPregunta}" (para mostrar en respuesta)`);
          }

          // Detectar mes(es) específico(s) - mejorado para detectar múltiples meses
          // Ejemplo: "cuanto debo del mes de junio, julio, agosto"
          const mesesRegex = /(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)/gi;
          const mesesEncontrados = preguntaParaMeses.match(mesesRegex);
          const mesesCapitalizados = mesesEncontrados ? mesesEncontrados.map(m => m.charAt(0).toUpperCase() + m.slice(1).toLowerCase()) : null;

          // Para compatibilidad con código existente, mantener mesEspecifico y mesCapitalizado
          const mesEspecifico = mesesCapitalizados && mesesCapitalizados.length === 1 ? mesesCapitalizados[0].toLowerCase() : null;
          const mesCapitalizado = mesesCapitalizados && mesesCapitalizados.length === 1 ? mesesCapitalizados[0] : null;

          const idsEstudiantes = estudiantesRelacionados.map(e => e.id).join(', ');

          console.log(`📅 Mes(es) detectado(s): ${mesesCapitalizados ? mesesCapitalizados.join(', ') : 'ninguno'}`);
          ...
          const estudianteIdUnico = estudiantesRelacionados.length === 1
            ? estudiantesRelacionados[0].id
            : (estudiantesRelacionados.length > 0 ? estudiantesRelacionados[0].id : null);

          if (!estudianteIdUnico && !usarVariosHijos) {
            throw new Error('No se encontró estudiante relacionado con tu número de teléfono.');
          }

          const whereEstudiantes = usarVariosHijos ? `ce.id_estudiante IN (${idsEstudiantes})` : `ce.id_estudiante = ${estudianteIdUnico}`;
          console.log(`👤 Usando estudiantes: ${usarVariosHijos ? `IN (${idsEstudiantes})` : estudianteIdUnico} (${estudiantesRelacionados.length} relacionado(s), nombres mencionados: ${nombresHijosMencionados.length})`);
          ...
          if (infoRemitente && mencionaPagos && estudiantesRelacionados.length === 0) {
            console.log('⚠️ No hay estudiantes relacionados pero hay infoRemitente. Intentando buscar estudiantes...');
            // El código ya debería haber buscado estudiantes arriba, pero si no los encontró, lanzar error más descriptivo
            throw new Error('No se encontraron estudiantes relacionados con tu número de teléfono. Por favor, verifica que tu número esté registrado correctamente en el sistema.');
          } else {
            throw new Error('La consulta SQL generada está incompleta. Debe incluir FROM y las tablas necesarias.');
          }
        }
      }

      // Si el SQL usa "IN (SELECT id FROM ESTUDIANTES)" sin filtro, reemplazar por los estudiantes del padre
      if (sqlGenerado && (sqlGenerado.includes('IN (SELECT id FROM ESTUDIANTES)') ||
        sqlGenerado.includes('IN (SELECT id FROM estudiantes)') ||
        sqlGenerado.includes('IN (SELECT id FROM `estudiantes`)'))) {
        console.error('❌ DETECTADO SQL que selecciona TODOS los estudiantes sin filtro');
        if (estudiantesRelacionados.length > 0) {
          const usarvarios = estudiantesRelacionados.length > 1 && nombresHijosMencionados.length >= 1;
          const idsJoin = estudiantesRelacionados.map(e => e.id).join(', ');
          const reemplazo = usarvarios ? `IN (${idsJoin})` : `= ${estudiantesRelacionados[0].id}`;
          console.log(`🔧 Reemplazando por estudiantes: ${reemplazo} (${estudiantesRelacionados.length} relacionado(s))`);

          sqlGenerado = sqlGenerado.replace(/IN\s*\(\s*SELECT\s+id\s+FROM\s+(?:ESTUDIANTES|estudiantes|`estudiantes`)\s*\)/gi, reemplazo);
          console.log(`✅ SQL corregido: ${sqlGenerado}`);
        } else {
          throw new Error('No se encontraron estudiantes relacionados con tu número de teléfono. El SQL intentó seleccionar todos los estudiantes, lo cual no está permitido para consultas de pagos personales.');
        }
      }
      ...
      const usarVariosHijosOverride = estudiantesRelacionados.length > 1 && (nombresHijosMencionados.length >= 1 || preguntaPorTodosLosHijos);
      if (estudiantesRelacionados.length > 1 && !usarVariosHijosOverride && sqlGenerado.includes('IN (')) {
        const estudianteIdUnicoOverride = estudiantesRelacionados[0].id;
        console.log(`⚠️ Múltiples estudiantes (${estudiantesRelacionados.length}), sin nombres mencionados. Filtrando SQL para usar solo ID: ${estudianteIdUnicoOverride}`);

        const patronIN = /IN\s*\([^)]+\)/gi;
        if (patronIN.test(sqlGenerado)) {
          sqlGenerado = sqlGenerado.replace(patronIN, `= ${estudianteIdUnicoOverride}`);
          console.log(`🔍 SQL modificado para un solo estudiante: ${sqlGenerado}`);
        }
      }
      ...
      const preguntaNormalizadaVencidas = normalizarTextoComparacion(pregunta || '');
      const esConsultaVencidas =
        /(vencid[oa]s?|atrasad[oa]s?|en mora|moros[oa]s?)/i.test(preguntaNormalizadaVencidas) &&
        /(detalle|detalles|mostrar|muestr|cual|cuales|ver|cuota|cuotas|pago|pagos)/i.test(preguntaNormalizadaVencidas);

      if (esConsultaVencidas && estudiantesRelacionados.length > 0) {
        const usarVariosHijosVencidas =
          estudiantesRelacionados.length > 1 && (nombresHijosMencionados.length >= 1 || preguntaPorTodosLosHijos);
        const idsEstudiantesVencidas = estudiantesRelacionados.map(e => e.id).join(', ');
        ...
        console.log('🔄 SQL forzado para detalle de cuotas vencidas:', sqlGenerado);
      }
      ...
      if (sqlGenerado && sqlGenerado.includes(condicionSaldoPendiente) && /\bwhere\b/i.test(sqlGenerado)) {
        const whereConAgregado = new RegExp(`\\s+AND\\s+\\(${condicionSaldoPendiente.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\)`, 'i');
        if (whereConAgregado.test(sqlGenerado)) {
          ...
          if (!/\bHAVING\b/i.test(sqlGenerado)) {
            if (/\bORDER BY\b/i.test(sqlGenerado)) {
              sqlGenerado = sqlGenerado.replace(/\bORDER BY\b/i, ` HAVING ${condicionSaldoPendiente} ORDER BY`);
            } else {
              sqlGenerado = `${sqlGenerado} HAVING ${condicionSaldoPendiente}`;
            }
          }
          console.log('🩹 SQL corregido automáticamente (SUM en WHERE -> HAVING).');
        }
      }
      ...
      try {
        ...
        console.log(`✅ SQL ejecutado exitosamente. Resultados: ${resultados.length} fila(s)`);
      ...
      if (!resultados || resultados.length === 0) {
        console.log(`⚠️ No se encontraron resultados para la consulta SQL: ${sqlGenerado}`);
        console.log(`📋 Info remitente disponible: ${infoRemitente ? 'Sí' : 'No'}`);
        console.log(`📋 Estudiantes relacionados: ${estudiantesRelacionados.length}`);
        if (estudiantesRelacionados.length > 0) {
          console.log(`📋 IDs de estudiantes: ${estudiantesRelacionados.map(e => e.id).join(', ')}`);
        }

        // Si es una pregunta sobre pagos y hay estudiantes relacionados, dar mensaje más útil
        const preguntaLower = pregunta.toLowerCase();
        const esPreguntaPagos = preguntaLower.includes('cuanto debo') || preguntaLower.includes('cuánto debo') ||
          preguntaLower.includes('pago') || preguntaLower.includes('cuota') ||
          preguntaLower.includes('mensualidad');
        const esPreguntaVencidas = /(vencid[oa]s?|atrasad[oa]s?|en mora|moros[oa]s?)/i.test(pregunta);

        if (esPreguntaVencidas && estudiantesRelacionados.length > 0) {
          return `No encontré cuotas vencidas con saldo pendiente para este estudiante en este momento.\n\n` +
        ...
        if (esPreguntaPagos && estudiantesRelacionados.length > 0) {
          return `No se encontraron registros de pagos mensuales para los estudiantes relacionados con tu número. Esto puede deberse a que:\n\n` +
      ...
      if (resultados.length === 1 && Object.keys(resultados[0]).length === 1) {
        ...
        const esConsultaInscripcionPrecio =
          /(inscrip|inscrib|matr[ií]cul|precio|costo|cu[aá]nto\s+cuesta|cuanto\s+cuesta|valor)/i.test(preguntaLowerSimple) &&
          /(kinder|pre[\s-]?kinder|inicial|primaria|secundaria|nivel|curso)/i.test(preguntaLowerSimple);
        ...
        if (esConsultaInscripcionPrecio) {
          ...
          if (/pre[\s-]?kinder/i.test(preguntaLowerSimple)) nivelBuscado = 'prekinder';
          else if (/kinder/i.test(preguntaLowerSimple)) nivelBuscado = 'kinder';
          else if (/inicial/i.test(preguntaLowerSimple)) nivelBuscado = 'inicial';
          else if (/primaria/i.test(preguntaLowerSimple)) nivelBuscado = 'primaria';
          else if (/secundaria/i.test(preguntaLowerSimple)) nivelBuscado = 'secundaria';
          ...
          if (nivelBuscado) {
            try {
              ...
              try {
                [detalleNivel] = await this.pool.query(
                  `SELECT
                   ...
                   GROUP BY n.id, n.nombre, n.descripcion, n.precio, n.meses, n.hora_entrada, n.hora_salida
                   ORDER BY n.id ASC
                   LIMIT 1`,
              ...
              if (detalleNivel && detalleNivel.length > 0) {
                ...
                const precio = parseFloat(n.precio || Object.values(resultados[0])[0] || 0) || 0;
                let mesesArr = [];
                try {
                ...
                const horaEntrada = n.hora_entrada ? String(n.hora_entrada).slice(0, 5) : null;
                const horaSalida = n.hora_salida ? String(n.hora_salida).slice(0, 5) : null;
                const nombreNivel = n.nombre || nivelBuscado.toUpperCase();

                let saludoCorto = 'Con gusto';
                if (infoRemitente?.nombre_padre) saludoCorto = `Claro Sr. ${infoRemitente.nombre_padre}`;
                else if (infoRemitente?.nombre_madre) saludoCorto = `Claro Sra. ${infoRemitente.nombre_madre}`;
              ...
              console.warn('⚠️ No se pudo construir respuesta detallada de nivel:', detalleErr.message);
            }
          }
        }
      ...
      const pareceConsultaNivelInformativa =
        /(precio|costo|cu[aá]nto\s+cuesta|cuanto\s+cuesta|arancel|mensualidad|horario|hora de entrada|hora de salida|turno)/i.test(preguntaLowerFormato) &&
        /(kinder|pre[\s-]?kinder|inicial|primaria|secundaria|nivel|curso)/i.test(preguntaLowerFormato);
      ...
      if (pareceConsultaNivelInformativa && tieneCamposNivelEnResultado) {
        ...
        if (/pre[\s-]?kinder/i.test(preguntaLowerFormato)) nivelDetectado = 'prekinder';
        else if (/kinder/i.test(preguntaLowerFormato)) nivelDetectado = 'kinder';
        else if (/inicial/i.test(preguntaLowerFormato)) nivelDetectado = 'inicial';
        else if (/primaria/i.test(preguntaLowerFormato)) nivelDetectado = 'primaria';
        else if (/secundaria/i.test(preguntaLowerFormato)) nivelDetectado = 'secundaria';

        let detalleNivel = resultados[0];

        // Si el SQL vino incompleto (ej: solo precio/hora_entrada), completar con datos de nivel.
        const faltanDatosClave = !detalleNivel.descripcion || !detalleNivel.meses || !detalleNivel.nombre;
        if (nivelDetectado && faltanDatosClave) {
          try {
            ...
            console.warn('⚠️ No se pudo completar detalle de nivel para formato informativo:', errNivel.message);
          }
        }

        const nombreNivel = (detalleNivel.nombre || detalleNivel.nivel_nombre || (nivelDetectado ? nivelDetectado.toUpperCase() : 'este nivel')).toString().toUpperCase();
        const descripcion = detalleNivel.descripcion ? String(detalleNivel.descripcion) : null;
        const precio = parseFloat(detalleNivel.precio || 0) || 0;

        let mesesCantidad = null;
        try {
          if (detalleNivel.meses) {
            const arrMeses = Array.isArray(detalleNivel.meses) ? detalleNivel.meses : JSON.parse(detalleNivel.meses);
        ...
        const horaEntrada = detalleNivel.hora_entrada ? String(detalleNivel.hora_entrada).slice(0, 5) : null;
        const horaSalida = detalleNivel.hora_salida ? String(detalleNivel.hora_salida).slice(0, 5) : null;
        const preguntaPideHorario = /(hora|horario|entrada|salida|turno)/i.test(preguntaLowerFormato);
        ...
        if (horaEntrada || horaSalida) {
          if (horaEntrada && horaSalida) respuestaNivel += `🕒 Horario referencial: ${horaEntrada} - ${horaSalida}\n`;
          else if (horaEntrada) respuestaNivel += `🕒 Hora de entrada: ${horaEntrada}\n`;
          else if (horaSalida) respuestaNivel += `🕒 Hora de salida: ${horaSalida}\n`;
          ...
          const turnoNorm = turnoTexto.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
          if (turnoNorm.includes('manana')) {
      ...
      if (resultados.length > 0 && resultados[0].nombre_mes) {
        ...
        if (resultados.length >= 2 && estudiantesRelacionados.length >= 2 && (resultados[0].id_estudiante == null || resultados[0].id_estudiante === undefined)) {
          resultados.forEach((r, i) => {
            if (i < estudiantesRelacionados.length) r.id_estudiante = estudiantesRelacionados[i].id;
          });
        }
        ...
        const esPreguntaMesEspecifico =
          ...
          /(cuándo vence|cuando vence|vencimiento).*(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)/i.test(preguntaParaFormato) ||
          /(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre).*(cuándo vence|cuando vence|vencimiento)/i.test(preguntaParaFormato) ||
          // "¿cuándo debo pagar en mayo?" / "¿hasta cuándo puedo pagar mayo?"
          /((cuándo|cuando)\s+debo\s+pagar|(hasta\s+cu[aá]ndo\s+(puedo|debo)\s+pagar)).*(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)/i.test(preguntaParaFormato) ||
          /(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre).*((cuándo|cuando)\s+debo\s+pagar|(hasta\s+cu[aá]ndo\s+(puedo|debo)\s+pagar))/i.test(preguntaParaFormato);
        ...
        const esPreguntaCuandoPagarMes =
          /((cuándo|cuando).*(pagar).*(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre))/i.test(preguntaParaFormato) ||
          /((febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre).*(cuándo|cuando).*(pagar))/i.test(preguntaParaFormato);

        const esPreguntaVencimientoOMesEspecifico = esPreguntaMesEspecifico || esPreguntaCuandoPagarMes;
        ...
        const esPreguntaCuantoDeboMes = /(cuánto|cuanto)\s+(debo|tengo\s+que|hay\s+que)\s+(pagar\s+)?(del\s+mes\s+de|de|en)\s+(mes\s+de\s+)?(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)/i.test(preguntaParaFormato);

        // Detectar si menciona múltiples meses
        const mesesRegex = /(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)/gi;
        const mesesMencionados = preguntaParaFormato.match(mesesRegex);
        const esPreguntaMultiplesMeses = mesesMencionados && mesesMencionados.length > 1;
        ...
        if (infoRemitente && estudiantesRelacionados.length > 0) {
          ...
          nombreEstudiante = `${estudiante.nombre} ${estudiante.apellido_paterno || ''} ${estudiante.apellido_materno || ''}`.trim();

          if (infoRemitente.nombre_padre) {
            nombrePadre = `Sr. ${infoRemitente.nombre_padre} ${infoRemitente.apellido_padre || ''}`.trim();
          } else if (infoRemitente.nombre_madre) {
            nombrePadre = `Sra. ${infoRemitente.nombre_madre} ${infoRemitente.apellido_madre || ''}`.trim();
        ...
        const idsUnicos = [...new Set(resultados.map(r => r.id_estudiante).filter(Boolean))];
        const porEstudiante = {};
        ...
        const idsEnResultados = estudiantesRelacionados
          .map(e => e.id)
          .filter(id => idsUnicos.includes(id) || idsUnicos.includes(String(id)));
        const idsFaltantes = idsUnicos.filter(id => !idsEnResultados.includes(id) && !idsEnResultados.includes(Number(id)));
        idsEnResultados.push(...idsFaltantes);
        const multiplesEstudiantes = idsEnResultados.length > 1;

        let respuesta = '';

        const prefijoMesCorregido = mesCorregidoPregunta ? `_(Refiriéndose a *${mesCorregidoPregunta}*)_\n\n` : '';
        if (multiplesEstudiantes && nombrePadre) {
          ...
          if (esPreguntaCuantoDeboMes && !esPreguntaMultiplesMeses && resultados.length === 1) {
            respuesta = prefijoMesCorregido;
          ...
          if (esPreguntaCuantoDeboMes && !esPreguntaMultiplesMeses && resultados.length === 1) {
            ...
            respuesta = prefijoMesCorregido + `📊 Información de pagos para *${nombreEstudiante}* (meses consultados):\n\n`;
          } else {
            respuesta = (esPreguntaMesEspecifico && resultados.length === 1) ? prefijoMesCorregido : prefijoMesCorregido + `📊 Información de pagos para *${nombreEstudiante}*:\n\n`;
          }
        } else {
          if (esPreguntaCuantoDeboMes && !esPreguntaMultiplesMeses && resultados.length === 1) {
            respuesta = prefijoMesCorregido;
          } else if (esPreguntaMultiplesMeses) {
            respuesta = prefijoMesCorregido + `📊 Información de pagos (meses consultados):\n\n`;
          } else {
            respuesta = (esPreguntaMesEspecifico && resultados.length === 1) ? prefijoMesCorregido : prefijoMesCorregido + `📊 Información de pagos:\n\n`;
          }
        }

        // Diagnóstico (solo para este tipo de respuesta): ayuda a detectar por qué se queda "solo encabezado"
        console.log(`🧾 [FormatoPagos] flags: esPreguntaCuantoDeboMes=${esPreguntaCuantoDeboMes} esPreguntaMultiplesMeses=${!!esPreguntaMultiplesMeses} esPreguntaVencimientoOMesEspecifico=${esPreguntaVencimientoOMesEspecifico} resultados=${resultados.length}`);
        ...
        const agregarFilasPago = (filas, nombreParaFilas) => {
          ...
          let montoEsperado = parseFloat(row.monto_esperado || row.monto_esperado_mensual || row.monto || 0) || 0;
          ...
          let montoPagado = parseFloat(row.monto_pagado || row.total_pagado || 0) || 0;
          ...
          const montoPendienteCalculado = parseFloat(row.monto_pendiente) || (montoEsperado - montoPagado);
          const montoPendiente = isNaN(montoPendienteCalculado) ? Math.max(0, montoEsperado - montoPagado) : montoPendienteCalculado;
          ...
          // Log para debugging
          if (montoEsperado === 0) {
            console.warn(`⚠️ Monto esperado es 0 para ${mes}. Datos disponibles:`, JSON.stringify(row));
          }
          if (montoPagado > 0) {
            console.log(`✅ ${mes}: Esperado=${montoEsperado.toFixed(2)}, Pagado=${montoPagado.toFixed(2)}, Pendiente=${montoPendiente.toFixed(2)}, Estado=${estado}`);
          }
          ...
          if (esPreguntaCuantoDeboMes && filas.length === 1 && !esPreguntaMultiplesMeses) {
          ...
          } else if (esPreguntaVencimientoOMesEspecifico && filas.length === 1) {
        ...
        if (multiplesEstudiantes) {
          idsEnResultados.forEach((id, index) => {
            const idKey = typeof id === 'number' ? id : Number(id);
            const nombreHijo = mapIdNombre[id] || mapIdNombre[idKey] || mapIdNombre[String(id)] || 'Estudiante';
            const filas = porEstudiante[id] || porEstudiante[idKey] || porEstudiante[String(id)] || [];
        ...
        if (resultados.length === 1 && typeof respuesta === 'string') {
          const soloEncabezado = respuesta.trim().endsWith(':') || respuesta.trim().endsWith(':\n') || respuesta.trim().endsWith(':\r\n');
          if (soloEncabezado) {
            const row = resultados[0] || {};
            const mes = row.nombre_mes || 'este mes';
            const montoEsperado = parseFloat(row.monto_esperado || row.monto || 0) || 0;
            const montoPagado = parseFloat(row.monto_pagado || 0) || 0;
            const montoPendiente = parseFloat(row.monto_pendiente) || Math.max(0, montoEsperado - montoPagado);
            let fechaFormateada = '';
            if (row.fecha_vencimiento) {
              try {
                const fecha = new Date(row.fecha_vencimiento);
                if (!isNaN(fecha.getTime())) {
                  fechaFormateada = fecha.toLocaleDateString('es-BO', { year: 'numeric', month: 'long', day: 'numeric' });
                }
              } catch (_) { }
            }
        ...
        if ((!esPreguntaMesEspecifico || resultados.length > 1) || esPreguntaMultiplesMeses) {
          ...
          const totalPendiente = resultados.reduce((sum, row) => {
            const valorEsperado = parseFloat(row.monto_esperado || row.monto || 0) || 0;
            const valorPagado = parseFloat(row.monto_pagado || 0) || 0;
            const valorPendiente = parseFloat(row.monto_pendiente) || (valorEsperado - valorPagado);
      ...
      resultados.slice(0, 20).forEach((row, idx) => {
        if (row.nombre && (row.apellido_paterno || row.apellido_materno)) {
          const nombre = `${row.nombre} ${row.apellido_paterno || ''} ${row.apellido_materno || ''}`.trim();
          respuesta += `${idx + 1}. ${nombre}\n`;
        } else {
          const campos = Object.entries(row).slice(0, 3).map(([k, v]) => `${k}: ${v}`).join(', ');
      ...
      if (error.message.includes('SQL') || error.message.includes('syntax') || error.message.includes('Error en la consulta')) {
        console.log('⚠️ Error SQL detectado, intentando fallback al reglamento...');
        try {
          const herramientaReglamento = new HerramientaReglamento();
          const respuestaReglamento = await herramientaReglamento.ejecutar(pregunta);
...
class HerramientaNotificacion extends Herramienta {
  constructor(notificacionesService) {
    super('notificacion', 'Procesa comandos de notificación a padres y tutores');
...
class HerramientaAyuda extends Herramienta {
  constructor() {
    super('ayuda', 'Responde a saludos y proporciona información sobre las capacidades del agente');
  }
  ...
  async ejecutar(pregunta, infoRemitente = null, contextoHistorial = '', infoUsuario = null) {
    ...
    if (infoUsuario) {
      ...
      nombreUsuario = infoUsuario.nombre ? ` ${infoUsuario.nombre.split(' ')[0]}` : '';
      ...
      if (infoUsuario.rol === 'Administrador' || infoUsuario.rol === 'Director') {
      ...
      nombreUsuario = infoRemitente.nombre_padre || infoRemitente.nombre_madre || infoRemitente.nombre_autorizado || '';
      if (nombreUsuario) {
...
function extraerRangosFechasEspanol(texto) {
  ...
  const reDosDias = /(\d{1,2})\s*y\s*(\d{1,2})\s+de\s+(enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|setiembre|octubre|noviembre|diciembre)\s+de\s+(\d{4})/gi;
  let m;
  ...
  const reRango = /(del\s+)?(\d{1,2})\s+al\s+(\d{1,2})\s+de\s+(enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|setiembre|octubre|noviembre|diciembre)\s+de\s+(\d{4})/gi;
...
function detectarAudienciaReglamento(preguntaLower = '') {
  ...
  const preguntaPadres = /\bpadres?\b|\bmadres?\b|\bapoderad|\btutor(es)?\b|\bfamilia\b/.test(p);
  const preguntaEstudiantes = /\bestudiantes?\b|\balumnos?\b|\bniñ[oa]s?\b|\badolescentes?\b/.test(p);
...
function filtrarContextoReglamentoPorAudiencia(contexto, audiencia, preguntaLower = '') {
  ...
  const reExcluir = /(inscrip|inscrib|matric|rude|vacun|certificado de nacimiento|requisit|fechas?|calendario|traslad|transferen)/i;

  const rePadres = /(padre|madre|tutor|apoderad|junta escolar|consejo educativo|participacion|prohib|ingreso.*hora de clases|convivencia|disciplina|sancion|infraestructura|cuidado.*bienes)/i;
  const reEstudiantes = /(estudiante|alumno|adolesc|convivencia|disciplina|sancion|falta|deber|derecho|asistencia|puntualidad|uniforme|comportamiento)/i;
...
class HerramientaReglamento extends Herramienta {
  constructor() {
    super('reglamento', 'Responde preguntas sobre el reglamento académico, requisitos, fechas de inscripción, uniformes, etc.');
  }
  ...
  async ejecutar(pregunta, infoRemitente = null, contextoHistorial = '', infoUsuario = null) {
    try {
      ...
      const preguntaLower = pregunta.toLowerCase();
      const necesitaTabla = /qu[eé]|cu[áa]les?|tipos?|lista|requisitos?|documentos?/i.test(preguntaLower);
      const esPreguntaSobreFechas = /cu[áa]ndo|cuando|fecha|inicio|empiezan|comienzan|apertura|calendario/i.test(preguntaLower);
      const esPreguntaAmplia = /primeros?\s+art[íi]culos?|cu[áa]les?\s+son\s+los|reglamentos?\s+(para|dedicad|dirigid)|padres?|madres?|estudiantes?|apoderados?|tutores?/i.test(preguntaLower);
      const esPreguntaSobreBecas = /beca|descuento|beneficio/i.test(preguntaLower);
      ...
      const pidioDetalle = /(detalle|detall|explica|completo|toda la informaci[oó]n)/i.test(preguntaLower);
      ...
      const numChunks = esPreguntaSobreBecas
        ? 24
        : (audiencia ? 18 : (esPreguntaAmplia ? 18 : (necesitaTabla || esPreguntaSobreFechas ? 12 : 8)));

      let contexto = await recuperarContextoDesdeReglamento(pregunta, numChunks);
      if (audiencia) {
        contexto = filtrarContextoReglamentoPorAudiencia(contexto, audiencia, preguntaLower);
      }
      ...
      if (esConsultaAunInscribir && contexto && contexto.trim().length > 0) {
        ...
        const maxEnd = rangos.reduce((acc, r) => (acc && acc.end > r.end ? acc : r), null)?.end;
      ...
      if (esRequisitosInscripcion && !pidioDetalle) {
        ...
        const diceRegular = /\bregular\b|\bestudiante regular\b|\bantiguo\b|\bantigua\b/.test(norm);
        const diceTraslado = /\btraslado\b|\btransferencia\b|\bcambio de unidad\b|\bcambiar de unidad\b/.test(norm);
      ...
      if (infoRemitente) {
        const esInscripcionOMatricula = /(inscrip|inscrib|matr[ií]cul)/i.test(preguntaLower);
        const aclaraNuevo = /\bnuevo\b|\bestudiante\s+nuevo\b/i.test(preguntaLower);
        ...
        if (esInscripcionOMatricula && aclaraNuevo) {
          const nombreAdulto = infoRemitente.nombre_padre || infoRemitente.nombre_madre || infoRemitente.nombre_autorizado || '';
          const prefijo = infoRemitente.nombre_padre
            ? `Sr. ${infoRemitente.nombre_padre}`
            : (infoRemitente.nombre_madre ? `Sra. ${infoRemitente.nombre_madre}` : (nombreAdulto || 'Padre/Tutor'));
      ...
      const esBecasLista = /(qu[eé]|cu[áa]les?)\s+(becas?|descuentos?)\s+(existen|hay|ofrece)/i.test(preguntaLower) || /(becas?|descuentos?)\s+(existen|hay|que\s+ofrece)/i.test(preguntaLower);
...
- Presenta la respuesta como lista corta: 4 a 8 puntos máximo, con frases claras.` : '';
      ...
      const instruccionBreveWhatsApp = (esRequisitosInscripcion && !pidioDetalle) ? `
...
IMPORTANTE: LEE el CONTEXTO y responde con la INFORMACIÓN directamente. NO menciones nombres de archivos ni documentos (PDF, DOCX). Presenta las becas, descuentos, fechas, etc. como información de la institución. Responde COMPLETA y DETALLADA:`;
      ...
      const maxTokensBecas = esBecasLista ? 8000 : (esRequisitosInscripcion && !pidioDetalle ? 1400 : 4500);
      ...
      const articulosMencionados = (respuestaTrim.match(/Art[íi]culo\s*\d+|Art\.\s*\d+/gi) || []).length;
      const pareceIncompleta = !respuestaTrim.match(/[.!?]$/) || (esBecasLista && articulosMencionados < 6);

      if (pareceIncompleta && respuestaTrim.length > 100) {
        console.warn(`⚠️ Respuesta incompleta (${articulosMencionados} artículos), solicitando continuación...`);
        try {
...
function analizarSentimiento(pregunta, historialConversacion = []) {
  ...
  const palabrasFrustracion = [
    'no entiendo', 'no funciona', 'no sirve', 'no me ayuda',
    'hablar con alguien', 'hablar con una persona', 'hablar con secretaria',
    'no puedo', 'imposible', 'muy difícil', 'complicado',
    'molesto', 'enojado', 'frustrado', 'desesperado',
    'no me resuelve', 'no me ayuda', 'inútil', 'no sirve para nada'
  ];

  // Palabras clave de satisfacción
  const palabrasSatisfaccion = [
    'gracias', 'perfecto', 'excelente', 'muy bien',
    'genial', 'perfecto', 'exacto', 'correcto',
    'me ayudó', 'muy útil', 'muy bueno'
  ];

  // Detectar frustración
  const tieneFrustracion = palabrasFrustracion.some(palabra => preguntaLower.includes(palabra));

  // Detectar necesidad de escalamiento
  const necesitaEscalamiento = preguntaLower.includes('hablar con') ||
  ...
  const tieneSatisfaccion = palabrasSatisfaccion.some(palabra => preguntaLower.includes(palabra));
  ...
  if (historialConversacion && historialConversacion.length > 0) {
    const ultimosMensajes = historialConversacion.slice(-3).map(msg => msg.mensaje?.toLowerCase() || '');
  ...
  return {
    ...
    necesitaEscalamiento: necesitaEscalamiento || (tieneFrustracion && frustracionAcumulada >= 2),
    tieneSatisfaccion,
    frustracionAcumulada,
    nivelUrgencia: necesitaEscalamiento ? 'alto' : (tieneFrustracion ? 'medio' : 'bajo')
  };
}

// Generar respuesta empática cuando hay frustración
function generarRespuestaEmpatica(analisisSentimiento, respuestaOriginal) {
  if (!analisisSentimiento.tieneFrustracion && !analisisSentimiento.necesitaEscalamiento) {
  ...
  if (analisisSentimiento.necesitaEscalamiento) {
    ...
    prefijoEmpatico += 'Te puedo ayudar a contactar con secretaría para que te atiendan personalmente. ';
    prefijoEmpatico += '¿Te parece bien?\n\n';
    prefijoEmpatico += 'Mientras tanto, aquí está la información que pediste:\n\n';
...
function normalizarTextoComparacionConMeses(s) {
  ...
  const meses = [
    ['marsso', 'marzo'], ['marso', 'marzo'], ['febrer', 'febrero'],
    ['abrirl', 'abril'], ['septiembr', 'septiembre'], ['ociubre', 'octubre'], ['diciembr', 'diciembre']
  ];
  meses.forEach(([mal, bien]) => { t = t.replace(new RegExp(mal, 'gi'), bien); });
  return t;
}
...
function determinarLongitudRespuesta(pregunta, tipoConsulta) {
  const preguntaLower = pregunta.toLowerCase();
  const esConsultaPago = /(pagar|pago|pagos|debo|deuda|pendiente|cuota|mensualidad|vence|vencimiento)/i.test(preguntaLower);
  ...
  const requiereCorta = preguntaLower.includes('sí o no') ||
  ...
  const requiereDetallada = preguntaLower.includes('explica') ||
  ...
  if (tipoConsulta === 'reglamento' && esRequisitosInscripcion && !pidioDetalle) return 'corta';
...
function mejorarFormatoRespuesta(respuesta, tipoConsulta, longitud, preguntaOriginal = '') {
  ...
  try {
    ...
    if (pregNorm || pregNormMeses) {
      const lineas = String(respuesta || '').replace(/\r\n/g, '\n').split('\n');
      let i = 0;
      while (i < Math.min(3, lineas.length)) {
        ...
        const nextNorm = i + 1 < lineas.length ? normalizarTextoComparacion(lineas[i + 1]) : '';
        const repitePregunta = (lnNorm && (lnNorm === pregNorm || lnNorm.startsWith(pregNorm))) ||
          (lnNormMeses && pregNormMeses && (lnNormMeses === pregNormMeses || lnNormMeses.startsWith(pregNormMeses)));
...
const CONECTORES_PREGUNTAS = ['y', 'también', 'además', 'igualmente', 'asimismo', 'otra cosa', 'por otro lado'];
...
const TEMPLATES_RESPUESTA_MULTIPLE = {
  ...
  tres_o_mas: (respuestas) => {
    ...
    respuestas.forEach((r, idx) => {
      const emoji = ['1️⃣', '2️⃣', '3️⃣', '4️⃣', '5️⃣', '6️⃣', '7️⃣', '8️⃣', '9️⃣', '🔟'][idx] || `${idx + 1}.`;
      resultado += `${emoji} ${r}\n\n`;
    });
...
function detectarMultiplesPreguntas(mensaje) {
  ...
  console.log(`🔍 [detectarMultiples] Mensaje: "${mensajeOriginal.substring(0, 100)}..."`);
  console.log(`🔍 [detectarMultiples] Signos de interrogación encontrados: ${signosInterrogacion}`);
  ...
  if (primerSignoInterrogacion > 0) {
    const textoAntes = mensajeOriginal.substring(0, primerSignoInterrogacion).trim();
    const palabrasSaludo = ['hola', 'buenos días', 'buenas tardes', 'buenas noches', 'buen día'];
  ...
  partes.forEach((parte, idx) => {
    ...
    if (!pregunta.startsWith('¿') && idx > 0) {
      ...
      for (const conector of CONECTORES_PREGUNTAS) {
        const regex = new RegExp(`^(y\\s+|también\\s+|además\\s+|${conector}\\s+)`, 'i');
      ...
      if (tieneConector) {
        pregunta = pregunta.replace(/^(y\s+|también\s+|además\s+|igualmente\s+|asimismo\s+|otra\s+cosa,?\s+|por\s+otro\s+lado,?\s+)/i, '');
        pregunta = '¿' + pregunta;
      } else {
        pregunta = '¿' + pregunta;
      }
    ...
    if (pregunta.length > 3) { // Evitar fragmentos muy cortos
  ...
  if (preguntas.length < 2) {
    ...
    const preguntasAlt = mensajeOriginal.split(/¿/).filter(p => p.trim().length > 0);
  ...
  console.log(`🔍 [detectarMultiples] Preguntas detectadas: ${preguntas.length}`);
...
function clasificarPreguntasMultiples(preguntas, infoRemitente = null) {
  console.log(`🔍 [clasificarMultiples] Clasificando ${preguntas.length} preguntas`);

  const clasificaciones = preguntas.map((pregunta, idx) => {
    const clasificacion = clasificarConsulta(pregunta, infoRemitente);
    console.log(`   ${idx + 1}. "${pregunta.substring(0, 50)}..." -> ${clasificacion.tipo} (${clasificacion.confianza})`);
  ...
  clasificaciones.forEach((item, idx) => {
    if (idx > 0) {
      ...
      const usaPronombres = /(él|ella|esa|ese|esto|aquello|lo|la)/i.test(item.pregunta);
 ...
 */
async function procesarPreguntasEnContexto(preguntasClasificadas, pool, infoRemitente, contextoHistorial, infoUsuario) {
  console.log(`🔄 [procesarEnContexto] Procesando ${preguntasClasificadas.length} preguntas`);
  ...
  for (const item of preguntasClasificadas) {
    try {
      console.log(`🔄 [procesarEnContexto] Procesando pregunta ${item.orden + 1}: "${item.pregunta.substring(0, 50)}..."`);
      ...
      if (item.clasificacion.herramienta === 'fecha_hora') {
        ...
        respuesta = await herramienta.ejecutar(item.pregunta, { historial: contextoDinamico });
      } else if (item.clasificacion.herramienta === 'ayuda') {
        herramienta = new HerramientaAyuda();
        respuesta = await herramienta.ejecutar(item.pregunta, infoRemitente, contextoDinamico, infoUsuario);
      ...
      respuesta = respuesta.replace(/^(hola|buenos días|buenas tardes|buenas noches)[,!\s]*/i, '').trim();

      respuestas.push(respuesta);

      console.log(`✅ [procesarEnContexto] Pregunta ${item.orden + 1} procesada exitosamente`);

    } catch (error) {
      console.error(`❌ [procesarEnContexto] Error procesando pregunta ${item.orden + 1}:`, error.message);
      respuestas.push(`Lo siento, tuve un problema procesando esta pregunta: ${error.message}`);
    }
  }
 ...
 */
function generarRespuestaEstructurada(respuestas, preguntasOriginales, inicioConSaludo = '') {
  console.log(`📝 [generarEstructurada] Formateando ${respuestas.length} respuestas`);
  ...
  if (respuestas.length === 2) {
    return saludoInicial + TEMPLATES_RESPUESTA_MULTIPLE.dos_preguntas(respuestas[0], respuestas[1]);
  }
...
function normalizarNivel(texto) {
  const mapeo = {
    'primer': 'PRIMER', 'primero': 'PRIMER',
    'segundo': 'SEGUNDO',
    'tercer': 'TERCER', 'tercero': 'TERCER',
    'cuarto': 'CUARTO',
    'quinto': 'QUINTO',
    'sexto': 'SEXTO'
  };
  ...
  if (numero && (texto.toLowerCase().includes('nivel') || texto.toLowerCase().includes('grado'))) {
...
function extraerMensaje(pregunta) {
  ...
  const matchQue = pregunta.match(/que\s+(.+?)\s+(?:a\s+todos|para|del?|en|primer|segundo|tercer)/i);
  if (matchQue) return matchQue[1];

  const matchMensaje = pregunta.match(/mensaje\s+(.+?)\s+(?:a\s+todos|para|del?|primer|segundo|tercer)/i);
...
function extraerParametrosEnvioMasivo(pregunta) {
  ...
  const esGeneral = /(a\s+todos|todos\s+los\s+niveles|toda\s+la\s+unidad|todos\s+los\s+padres(?!\s+de)|general)/i.test(pregunta)
    && !/(de\s+primer|de\s+segundo|del\s+primer|del\s+segundo|para\s+primer|para\s+segundo)/i.test(pregunta);
  ...
  const niveles = [];
  const patronMultiple = /(primer|primero|segundo|tercer|tercero|cuarto|quinto|sexto)\s+(nivel|grado)/gi;
  ...
  console.log(`📢 Detectados ${niveles.length} nivel(es): ${niveles.join(', ')}`);
...
async function obtenerPadresPorNivel(niveles, pool) {
  try {
    ...
    const nivelesTexto = niveles === null ? 'TODOS los niveles' : niveles.join(', ');
    console.log(`📊 Encontrados ${padres.length} contactos verificados en ${nivelesTexto} (${anioActual})`);
...
function personalizarMensaje(template, padre) {
  ...
  const nombreEstudiante = `${padre.estudiante_nombre} ${padre.estudiante_apellido_paterno || ''}`.trim();
  const mensaje = `Hola ${nombreTutor} ${apellidoTutor}, quería informarle que ${template}. `
...
function normalizarMesesEnPregunta(pregunta) {
  if (!pregunta || typeof pregunta !== 'string') return { preguntaNormalizada: pregunta, mesCorregido: null };
  const sustituciones = [
...
function clasificarConsulta(pregunta, infoRemitente = null) {
  ...
  if (infoRemitente) {
    console.log(`🔍 [clasificarConsulta] PRIORIDAD MÁXIMA: Verificando pagos personales para padre/tutor`);

    // Detectar patrones de pagos con máxima prioridad
    const mencionaDebo = /(cuánto|cuanto|cuándo|cuando).*debo/i.test(pregunta);
    const mencionaMesEspecifico = /(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)/i.test(pregunta);
    const mencionaHijo = /(mi hija|mi hijo|mis hijos|mis hijas|de mi hija|de mi hijo|de mis hijos|de mis hijas|mi estudiante|mis estudiantes|mi niño|mi niña|mis niños|mis niñas)/i.test(pregunta);
    const mencionaPagos = /(pago|cuota|mensualidad|debo|deuda)/i.test(pregunta);
    const mencionaVencidas = /(vencid[oa]s?|atrasad[oa]s?|en\s+mora|moros[oa]s?)/i.test(pregunta);
    const pideDetalleVencidas = /(detalle|detalles|mostrar|muestrame|muéstrame|cual|cuales|ver)/i.test(preguntaLower);

    console.log(`  [clasificarConsulta] - mencionaDebo: ${mencionaDebo}`);
    console.log(`  [clasificarConsulta] - mencionaMesEspecifico: ${mencionaMesEspecifico}`);
    console.log(`  [clasificarConsulta] - mencionaHijo: ${mencionaHijo}`);
    console.log(`  [clasificarConsulta] - mencionaPagos: ${mencionaPagos}`);
    console.log(`  [clasificarConsulta] - mencionaVencidas: ${mencionaVencidas}`);
    console.log(`  [clasificarConsulta] - pideDetalleVencidas: ${pideDetalleVencidas}`);

    // CRÍTICO: frases tipo "si cuales se han vencido" / "muestrame las vencidas"
    if (mencionaVencidas && (pideDetalleVencidas || mencionaPagos || mencionaDebo)) {
      console.log(`✅ [clasificarConsulta] PRIORIDAD MÁXIMA: detalle de cuotas vencidas detectado - RETORNANDO base_datos`);
    ...
    if (mencionaDebo && mencionaMesEspecifico && mencionaHijo) {
      console.log(`✅ [clasificarConsulta] PRIORIDAD MÁXIMA: "cuanto debo" + mes + hijo detectado - RETORNANDO base_datos`);
      return { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.99 };
    }

    // CRÍTICO: Si menciona "cuanto debo" + mes, es SIEMPRE base_datos
    if (mencionaDebo && mencionaMesEspecifico) {
      console.log(`✅ [clasificarConsulta] PRIORIDAD ALTA: "cuanto debo" + mes detectado - RETORNANDO base_datos`);
    ...
    if (mencionaPagos && mencionaMesEspecifico && mencionaHijo) {
      console.log(`✅ [clasificarConsulta] PRIORIDAD ALTA: pagos + mes + hijo detectado - RETORNANDO base_datos`);
      return { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.98 };
    }

    // Si menciona "cuanto debo" (sin mes específico pero con hijo), también es base_datos
    if (mencionaDebo && mencionaHijo) {
      console.log(`✅ [clasificarConsulta] PRIORIDAD ALTA: "cuanto debo" + hijo detectado - RETORNANDO base_datos`);
  ...
  const palabrasSaludos = [
    ...
    'saludos', 'qué tal', 'como estas', 'cómo estás', 'hey', 'hi'
  ];

  const palabrasAyuda = [
    'ayuda', 'ayúdame', 'help', 'qué puedes hacer', 'que puedes hacer',
    'qué sabes', 'que sabes', 'para qué sirves', 'para que sirves',
    'cómo funciona', 'como funciona', 'qué opciones', 'que opciones',
    'menú', 'menu', 'opciones disponibles', 'qué puedo preguntar',
    'que puedo preguntar', 'qué consultas', 'que consultas'
  ];
  ...
  const mencionaPagosDespuesSaludo = /(cuanto|cuánto|pago|cuota|mensualidad|debo|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre|mi hijo|mi hija)/i.test(pregunta);
  const esSoloSaludo = tieneSaludo && preguntaLower.length < 25 && !mencionaPagosDespuesSaludo;
  ...
  const patronEnvioMasivo = /(envía|envia|envíe|enviar|manda|mandar).*mensaje.*a.*(todos|padres|tutores|nivel)/i;
  ...
  const palabrasNotificacion = [
    'notificar', 'notifica', 'notifique', 'enviar comunicado',
    'comunicar', 'comunica', 'avisar', 'avisa',
    'comunicado', 'anuncio', 'anunciar',
    'informar', 'enviar aviso', 'circular', 'memo', 'memorandum'
  ];

  const esComandoNotificacion = palabrasNotificacion.some(p => preguntaLower.includes(p));
  if (esComandoNotificacion) {
    return { tipo: 'notificacion', herramienta: 'notificacion', confianza: 0.95 };
  }
  ...
  const consultaHorarioEstudiante =
    /(a\s+qu[eé]\s+hora|hora\s+de\s+entrada|hora\s+de\s+salida|horario|entra|sale)/i.test(preguntaLower) &&
    /(mi\s+hij[oa]s?|mis\s+hij[oa]s?|mi\s+estudiante|mis\s+estudiantes|hij[oa]s?)/i.test(preguntaLower);
  ...
  const mencionaCostoONivel =
    /(precio|costo|cu[aá]nto\s+cuesta|cuanto\s+cuesta|valor|mensualidad|arancel)/i.test(preguntaLower) &&
    /(kinder|pre[\s-]?kinder|inicial|primaria|secundaria|nivel|curso|inscrip|inscrib|matr[ií]cul)/i.test(preguntaLower);

  const mencionaHorarioEscolar =
    /(hora\s+de\s+entrada|hora\s+de\s+salida|a\s+qu[eé]\s+hora\s+(entra|sale|es\s+la\s+entrada)|horario|turno)/i.test(preguntaLower) &&
    /(kinder|pre[\s-]?kinder|inicial|primaria|secundaria|nivel|curso|inscrip|inscrib|matr[ií]cul|entrada|salida)/i.test(preguntaLower);
  ...
  const palabrasFechaHora = [
    'hoy', 'fecha', 'día', 'día es', 'hora', 'qué día', 'que dia',
    'qué hora', 'que hora', 'fecha actual', 'día actual', 'ahora',
    'momento actual', 'qué fecha', 'que fecha'
  ];
  ...
  const pareceFueraDeFecha = /fuera\s+de\s+fecha|fecha\s+de\s+inscrip|fechas?\s+de\s+inscrip/i.test(preguntaLower);

  if (!pareceInscripcion && !pareceFueraDeFecha && palabrasFechaHora.some(p => preguntaLower.includes(p))) {
  ...
  if (infoRemitente) {
    console.log(`🔍 [clasificarConsulta] Verificando clasificación para padre/tutor. Pregunta: "${pregunta}"`);

    // Detectar si menciona meses específicos O cuotas/mensualidades O vencimientos
    const mencionaMesEspecifico = /(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)/i.test(pregunta);
    const mencionaCuotaOMensualidad = /(cuota|cuotas|mensualidad|mensualidades|pago|pagos|deuda|deudas|pendiente|pendientes)/i.test(pregunta);
    const mencionaVencimiento = /(cuándo vence|cuando vence|vencimiento|fecha.*vencimiento|vencid[oa]s?|se han vencido)/i.test(pregunta);
    const mencionaHijo = /(mi hija|mi hijo|mis hijos|mis hijas|de mi hija|de mi hijo|de mis hijos|de mis hijas|mi estudiante|mis estudiantes|mi niño|mi niña|mis niños|mis niñas)/i.test(pregunta);
    ...
    const patronVencimientoCuotaMesHijo = /(cuándo vence|cuando vence).*(cuota|mensualidad|pago).*(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre).*(mi hija|mi hijo|mis hijos|mis hijas|de mi hija|de mi hijo|de mis hijos|de mis hijas)/i.test(pregunta) ||
      /(cuota|mensualidad|pago).*(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre).*(mi hija|mi hijo|mis hijos|mis hijas|de mi hija|de mi hijo|de mis hijos|de mis hijas).*(cuándo vence|cuando vence|vencimiento)/i.test(pregunta);

    console.log(`  [clasificarConsulta] - Menciona mes específico: ${mencionaMesEspecifico}`);
    console.log(`  [clasificarConsulta] - Menciona cuota/mensualidad: ${mencionaCuotaOMensualidad}`);
    console.log(`  [clasificarConsulta] - Menciona vencimiento: ${mencionaVencimiento}`);
    console.log(`  [clasificarConsulta] - Menciona hijo: ${mencionaHijo}`);
    console.log(`  [clasificarConsulta] - Menciona "debo": ${mencionaDebo}`);
    console.log(`  [clasificarConsulta] - Patrón vencimiento+cuota+mes+hijo: ${patronVencimientoCuotaMesHijo}`);

    // CRÍTICO: Patrón específico tiene máxima prioridad
    if (patronVencimientoCuotaMesHijo) {
      console.log(`✅ Clasificado como base_datos - Patrón específico detectado (vencimiento + cuota + mes + hijo)`);
      return { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.99 };
    }

    // CRÍTICO: Detectar "cuanto debo del mes de X de mi hijo" - máxima prioridad
    if (mencionaDebo && mencionaMesEspecifico && mencionaHijo) {
      console.log(`✅ [clasificarConsulta] Clasificado como base_datos - Patrón "cuanto debo del mes de X de mi hijo" detectado`);
      return { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.99 };
    }

    // CRÍTICO: Si hay infoRemitente y menciona "cuanto debo" + mes, SIEMPRE es base_datos
    if (mencionaDebo && mencionaMesEspecifico) {
      console.log(`✅ [clasificarConsulta] Clasificado como base_datos - Padre/tutor pregunta "cuanto debo del mes de X"`);
      return { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.98 };
    }
    ...
    if (mencionaHijo && (mencionaMesEspecifico || mencionaCuotaOMensualidad || mencionaVencimiento)) {
      console.log(`✅ Clasificado como base_datos - Padre/tutor pregunta sobre pagos específicos (hijo + mes/cuota/vencimiento)`);
      return { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.98 };
    }
    ...
    if (mencionaMesEspecifico && (mencionaCuotaOMensualidad || mencionaVencimiento)) {
      console.log(`✅ Clasificado como base_datos - Padre/tutor pregunta sobre pagos específicos (mes + cuota/vencimiento)`);
      return { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.97 };
    }

    const palabrasPagosPersonales = [
      // Referencias directas a hijos
      'mi hija', 'mi hijo', 'mis hijos', 'mis hijas', 'mi estudiante', 'mis estudiantes', 'mi niño', 'mi niña', 'mis niños', 'mis niñas',
      'de mi hija', 'de mi hijo', 'de mis hijos', 'de mis hijas', 'mi wawa', 'mi nena', 'mi nene',
      // Pagos propios
      'mis pagos', 'mis mensualidades', 'mis cuotas', 'mi deuda', 'mis deudas',
      // Cuánto debo (variaciones bolivianas)
      'cuánto debo', 'cuanto debo', 'cuándo debo', 'cuando debo', 'cuánto debo pagar', 'cuanto debo pagar', 'cuándo debo pagar', 'cuando debo pagar',
      'cuánto debo del mes', 'cuanto debo del mes', 'cuánto debo de', 'cuanto debo de',
      'cuánto debo del mes de', 'cuanto debo del mes de', 'cuánto debo de febrero', 'cuanto debo de febrero',
      'cuánto debo de marzo', 'cuanto debo de marzo', 'cuánto debo de abril', 'cuanto debo de abril',
      'cuánto debo de mayo', 'cuanto debo de mayo', 'cuánto debo de junio', 'cuanto debo de junio',
      'cuánto debo de julio', 'cuanto debo de julio', 'cuánto debo de agosto', 'cuanto debo de agosto',
      'cuánto debo de septiembre', 'cuanto debo de septiembre', 'cuánto debo de octubre', 'cuanto debo de octubre',
      'cuánto debo de noviembre', 'cuanto debo de noviembre', 'cuánto debo de diciembre', 'cuanto debo de diciembre',
      'cuánto toca pagar', 'cuanto toca pagar', 'cuánto me toca', 'cuanto me toca',
      'cuánto toca', 'cuanto toca', 'cuánto es', 'cuanto es',
      'cuánto sale', 'cuanto sale', 'cuánto cuesta', 'cuanto cuesta',
      'cuánto va a ser', 'cuanto va a ser', 'cuánto sería', 'cuanto seria',
      // Pagos realizados
      'cuánto pago', 'cuanto pago', 'cuánto pagué', 'cuanto pague',
      'cuánto he pagado', 'cuanto he pagado', 'ya pagué', 'ya pague',
      // Qué debo
      'qué debo', 'que debo', 'qué debo pagar', 'que debo pagar',
      'qué me falta', 'que me falta', 'qué falta pagar', 'que falta pagar',
      'cuánto falta', 'cuanto falta', 'cuánto me falta', 'cuanto me falta',
      // Vencimientos (CRÍTICO - debe detectar estas frases)
      'cuándo vence', 'cuando vence', 'cuándo tengo que pagar', 'cuando tengo que pagar',
      'fecha límite', 'fecha limite', 'hasta cuándo', 'hasta cuando',
      'próximo pago', 'proximo pago', 'siguiente pago',
      'cuándo vence la', 'cuando vence la', 'cuándo vence el', 'cuando vence el',
      'vencimiento', 'vencimiento de', 'vencimiento del',
      // Meses específicos (CRÍTICO para detectar consultas sobre meses)
      'cuota de febrero', 'cuota de marzo', 'cuota de abril', 'cuota de mayo',
      'cuota de junio', 'cuota de julio', 'cuota de agosto', 'cuota de septiembre',
      'cuota de octubre', 'cuota de noviembre', 'cuota de diciembre',
      'mensualidad de febrero', 'mensualidad de marzo', 'mensualidad de abril',
      'pago de febrero', 'pago de marzo', 'pago de abril',
      // Meses
      'cada qué mes', 'cada que mes', 'qué meses', 'que meses',
      'cuáles meses', 'cuales meses', 'qué meses faltan', 'que meses faltan',
      'meses pendientes', 'meses pagados', 'meses atrasados',
      // Estado
      'pagos pendientes', 'deuda', 'deudas', 'pendiente', 'pendientes',
      'estado de pagos', 'saldo', 'saldo pendiente', 'mora', 'en mora',
      'atrasado', 'atrasados', 'al día', 'al dia',
      // Nuevas palabras clave de pago
      'factura', 'facturas', 'recibo', 'recibos', 'comprobante', 'comprobantes',
      'pagar', 'cancelar', 'abonar', 'abono', 'pagado', 'cancelado', 'abonado',
      'cuánto es el total', 'cuanto es el total', 'total a pagar', 'total pendiente',
      'plan de pagos', 'plan de pago', 'cronograma de pagos', 'cuotas atrasadas',
      'pagos realizados', 'historial de pagos', 'estado de cuenta', 'mi estado de cuenta'
    ];
    ...
    const patronCuantoDeboMes = /(cuánto|cuanto|cuándo|cuando)\s+debo\s+.*?(del\s+mes\s+de|de)\s+(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)(.*(mi hija|mi hijo|mis hijos|mis hijas|de mi hija|de mi hijo|de mis hijos|de mis hijas))?/i.test(pregunta);

    // También detectar "cuanto debo del mes de febrero de mi hijo" (palabras en diferente orden)
    const patronCuantoDeboMesHijo = /(cuánto|cuanto|cuándo|cuando)\s+debo\s+.*?del\s+mes\s+de\s+(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)\s+de\s+(mi hijo|mi hija|mis hijos|mis hijas)/i.test(pregunta);

    // Detectar también variaciones como "cuanto debo del mes de febrero de mi hijo" sin "de" antes de "mi hijo"
    const patronCuantoDeboMesHijo2 = /(cuánto|cuanto|cuándo|cuando)\s+debo\s+.*?del\s+mes\s+de\s+(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre).*?(mi hijo|mi hija|mis hijos|mis hijas)/i.test(pregunta);

    console.log(`  - Patrón "cuanto debo del mes": ${patronCuantoDeboMes}`);
    console.log(`  - Patrón "cuanto debo del mes de X de mi hijo": ${patronCuantoDeboMesHijo}`);
    console.log(`  - Patrón "cuanto debo del mes de X mi hijo": ${patronCuantoDeboMesHijo2}`);

    const esConsultaPersonalPagos = palabrasPagosPersonales.some(p => preguntaLower.includes(p)) ||
      (mencionaMesEspecifico && (mencionaCuotaOMensualidad || mencionaVencimiento)) ||
      patronCuantoDeboMes ||
      patronCuantoDeboMesHijo ||
      patronCuantoDeboMesHijo2;

    if (esConsultaPersonalPagos) {
      console.log(`✅ [clasificarConsulta] Clasificado como base_datos - Consulta personal de pagos detectada`);
      console.log(`  [clasificarConsulta] - palabrasPagosPersonales match: ${palabrasPagosPersonales.some(p => preguntaLower.includes(p))}`);
      console.log(`  [clasificarConsulta] - mes + cuota/vencimiento: ${mencionaMesEspecifico && (mencionaCuotaOMensualidad || mencionaVencimiento)}`);
      console.log(`  [clasificarConsulta] - patronCuantoDeboMes: ${patronCuantoDeboMes}`);
      console.log(`  [clasificarConsulta] - patronCuantoDeboMesHijo: ${patronCuantoDeboMesHijo}`);
      console.log(`  [clasificarConsulta] - patronCuantoDeboMesHijo2: ${patronCuantoDeboMesHijo2}`);
      return { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.95 };
    } else {
      console.log(`⚠️ [clasificarConsulta] NO clasificado como base_datos para pagos personales`);
      console.log(`  [clasificarConsulta] - palabrasPagosPersonales match: ${palabrasPagosPersonales.some(p => preguntaLower.includes(p))}`);
      console.log(`  [clasificarConsulta] - mes + cuota/vencimiento: ${mencionaMesEspecifico && (mencionaCuotaOMensualidad || mencionaVencimiento)}`);
      console.log(`  [clasificarConsulta] - patronCuantoDeboMes: ${patronCuantoDeboMes}`);
      console.log(`  [clasificarConsulta] - patronCuantoDeboMesHijo: ${patronCuantoDeboMesHijo}`);
  ...
  const mencionaEstudianteEspecifico = patronesNombreEstudiante.some(patron => patron.test(pregunta));

  if (mencionaEstudianteEspecifico || preguntaLower.includes('pagos de') || preguntaLower.includes('pago de')) {
    return { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.95 };
  }
  ...
  const palabrasDatos = [
    // Conteos y listados
    'cuántos estudiantes', 'cuantos estudiantes', 'cuántos alumnos', 'cuantos alumnos',
    'cuántas inscripciones', 'cuantas inscripciones', 'cuántos hay', 'cuantos hay',
    'listar estudiantes', 'mostrar estudiantes', 'ver estudiantes', 'lista de estudiantes',
    'listar alumnos', 'mostrar alumnos', 'todos los estudiantes', 'todos los alumnos',
    'listado', 'reporte', 'resumen', 'estadísticas', 'estadisticas',
    // Deudas y pagos
    'deuda', 'deudas', 'pendiente de pago', 'pagos pendientes', 'morosos', 'morosidad',
    'en mora', 'atrasados', 'quiénes deben', 'quienes deben', 'quién debe', 'quien debe',
    // Sistema y usuarios
    'usuarios del sistema', 'estudiantes inscritos', 'total de', 'total inscritos',
    'cuántos usuarios', 'cuantos usuarios', 'roles', 'administradores',
    // Consultas de pagos específicas
    'cuánto debe', 'cuanto debe', 'cuánto pagó', 'cuanto pago',
    'cuánto debo', 'cuanto debo', 'cuánto tengo que pagar', 'cuanto tengo que pagar',
    'cuándo vence', 'cuando vence', 'fecha de vencimiento', 'fechas de vencimiento',
    'pagos de', 'pago de', 'estado de pago', 'estado de pagos',
    // Ingresos y finanzas
    'ingresos', 'recaudación', 'recaudacion', 'cobros', 'cobrado', 'recibido',
    'cuánto se ha cobrado', 'cuanto se ha cobrado', 'total cobrado', 'total recaudado',
    // Inscripciones y gestión
    'inscritos', 'inscripciones del', 'inscripciones de', 'matriculados',
    'nuevos estudiantes', 'este año', 'este año', 'gestión', 'gestion'
  ];

  const esBaseDatos = palabrasDatos.some(p => preguntaLower.includes(p));

  if (esBaseDatos) {
    return { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.90 };
  }
  ...
  const patronVencimientoCuotaMes = /(cuándo vence|cuando vence).*(cuota|mensualidad|pago).*(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)/i.test(pregunta) ||
    /(cuota|mensualidad|pago).*(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre).*(cuándo vence|cuando vence|vencimiento)/i.test(pregunta);

  const esSobrePagosEspecificos = patronVencimientoCuotaMes || (
    (preguntaLower.includes('pagos') || preguntaLower.includes('cuota') || preguntaLower.includes('mensualidad') ||
      preguntaLower.includes('vencimiento') || preguntaLower.includes('cuándo vence') || preguntaLower.includes('cuando vence') ||
      /vencid[oa]s?|atrasad[oa]s?|moros[oa]s?/.test(preguntaLower))
    && (
      preguntaLower.includes(' de ') || preguntaLower.includes(' del ') || preguntaLower.includes(' de la ') || preguntaLower.includes(' de los ') || preguntaLower.includes(' de las ') ||
      preguntaLower.includes(' mi ') || preguntaLower.includes(' de mi ') || preguntaLower.includes(' mis ') || preguntaLower.includes(' de mis ') ||
      /(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)/i.test(pregunta) ||
      /(mi hija|mi hijo|mis hijos|mis hijas|de mi hija|de mi hijo|de mis hijos|de mis hijas)/i.test(pregunta)
    )
  );
  ...
  if (infoRemitente && esSobrePagosEspecificos) {
    console.log(`✅ Clasificado como base_datos - Padre/tutor pregunta sobre pagos específicos (infoRemitente + pagos específicos)`);
    return { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.98 };
  }

  // Si menciona pagos específicos (incluso sin infoRemitente), también es base_datos
  if (esSobrePagosEspecificos) {
    console.log(`✅ Clasificado como base_datos - Consulta sobre pagos específicos detectada`);
    return { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.95 };
  }
  ...
  const palabrasReglamento = [
    ...
    'requisitos', 'requisito', 'inscribir', 'inscripción', 'inscripcion', 'inscribirse',
    'documentos', 'documento', 'papeles', 'papelería', 'papeleria',
    'necesito', 'necesita', 'traer', 'presentar', 'entregar',
    'qué piden', 'que piden', 'qué necesito', 'que necesito', 'qué hay que', 'que hay que',
    // Uniforme y vestimenta
    'uniforme', 'uniformes', 'vestimenta', 'ropa', 'buzo', 'chompa', 'polera',
    // Matrícula y costos generales
    'matrícula', 'matricula', 'costo de', 'precio de', 'cuánto cuesta inscribir',
    'cuanto cuesta inscribir', 'valor de', 'tarifa',
    // Horarios y turnos
    'turno', 'turnos', 'horario', 'horarios', 'clases', 'hora de entrada', 'hora de salida',
    'mañana', 'tarde', 'doble turno', 'jornada',
    // Fechas importantes (PERO NO fechas de vencimiento de cuotas específicas)
    'inicio', 'cuándo inicia', 'cuando inicia', 'cuándo empiezan', 'cuando empiezan',
    'inicio de clases', 'primer día', 'primer dia', 'apertura',
    'calendario', 'calendario escolar', 'año escolar',
    'vacaciones', 'feriados', 'receso', 'trimestre', 'bimestre',
    // Becas y descuentos
    'becas', 'beca', 'descuento', 'descuentos', 'rebaja', 'beneficio',
    'ayuda económica', 'ayuda economica', 'apoyo', 'subvención', 'subvencion',
    // Aranceles generales (NO específicos de un estudiante)
    'aranceles', 'arancel', 'pensión', 'pension', 'mensualidad regular', 'cuánto es la mensualidad',
    // Procesos
    'proceso', 'paso', 'pasos', 'cómo', 'como', 'procedimiento', 'trámite', 'tramite',
    'cómo hago', 'como hago', 'cómo puedo', 'como puedo', 'qué hago para', 'que hago para',
    // Normativa
    'reglamento', 'normas', 'norma', 'política', 'politica', 'reglas',
    'conducta', 'disciplina', 'sanciones', 'faltas',
    // Materiales
    'materiales', 'material', 'útiles', 'utiles', 'escolares', 'lista de materiales',
    'cuadernos', 'libros', 'textos',
    // Actividades
    'actividades', 'extracurriculares', 'deportes', 'talleres', 'cursos extra',
    'música', 'musica', 'danza', 'arte', 'inglés', 'ingles',
    // Servicios
    'transporte', 'movilidad', 'bus', 'almuerzo', 'comedor', 'refrigerio', 'lonchera',
    // Cambios y transferencias
    'transferencia', 'cambio', 'cambio de turno', 'cambio de curso', 'traslado',
    'reinscripción', 'reinscripcion', 're-inscripción', 'renovar inscripción',
    // Niveles educativos  
    'inicial', 'kinder', 'primaria', 'secundaria', 'prekinder', 'pre-kinder',
    'primer grado', 'segundo grado', 'nivel', 'grado', 'curso',
    // Otros
    'certificado', 'libreta', 'notas', 'calificaciones', 'boletín', 'boletin',
    'promoción', 'promocion', 'aprobación', 'aprobacion'
  ];

  // Solo usar reglamento si NO es sobre pagos específicos
  const esReglamento = palabrasReglamento.some(p => preguntaLower.includes(p));
  ...
  if (preguntaLower.length < 10) {
    return { tipo: 'ayuda', herramienta: 'ayuda', confianza: 0.60 };
  }
...
function detectarAclaracionInscripcion(pregunta, historialConversacion = []) {
  ...
  const seleccion = mapa.get(txt) || (txt.length <= 12 ? (mapa.get(txt.split(' ')[0]) || null) : null);
  if (!seleccion) return null;

  if (!Array.isArray(historialConversacion) || historialConversacion.length === 0) return null;
  ...
  const idxUltimaPreguntaBot = (() => {
    for (let i = ultimos.length - 1; i >= 0; i--) {
      ...
      const preguntaTipoEstudianteClasica =
        ...
        (t.includes('nuevo') && t.includes('regular') && t.includes('es estudiante')) ||
        (t.includes('nuevo') && t.includes('regular') && t.includes('es') && t.includes('estudiante'));

      // Soportar variantes que ya usa el bot:
      // "¿Su caso es nuevo, regular o traslado?"
      const preguntaTipoEstudianteConTraslado =
        (t.includes('su caso') && t.includes('nuevo') && t.includes('regular')) ||
        (t.includes('nuevo') && t.includes('regular') && t.includes('traslado')) ||
        (t.includes('nuevo') && t.includes('regular') && t.includes('transferencia'));
  ...
  for (let i = idxUltimaPreguntaBot; i >= 0; i--) {
    ...
    const t = normalizarTextoComparacion(m.mensaje);
    if (t.includes('requisit') && (t.includes('inscrip') || t.includes('inscrib') || t.includes('matric'))) {
      preguntaOriginal = m.mensaje;
      break;
    }
  }

  const preguntaReescrita = (preguntaOriginal && String(preguntaOriginal).trim().length > 0)
  ...
  return {
    seleccion,
    preguntaReescrita,
    clasificacionForzada: { tipo: 'reglamento', herramienta: 'reglamento', confianza: 0.92 }
  };
}
...
function detectarAclaracionDetallesPagos(pregunta, historialConversacion = []) {
  ...
  const esAfirmacionCorta = /\b(si|sí|claro|ok|dale|de acuerdo|correcto)\b/.test(txt);
  ...
  const preguntaPorVencidos = /\b(cual|cuales)\b/.test(txt) && /\b(vencid)\b/.test(txt);
  ...
  return {
    preguntaReescrita: 'Muéstrame el detalle de mis cuotas vencidas, incluyendo mes, fecha de vencimiento, monto esperado, monto pagado y monto pendiente.',
    clasificacionForzada: { tipo: 'base_datos', herramienta: 'base_datos', confianza: 0.96 }
  };
}

// Ejecutar agente
async function ejecutarAgente(pregunta, pool, usuarioId = null, infoRemitente = null, historialConversacion = [], infoUsuario = null) {
  ...
  try {
    ...
    if (preguntaNormalizada && preguntaNormalizada !== pregunta) {
      console.log(`📝 [ejecutarAgente] Pregunta normalizada (meses): "${preguntaNormalizada}"`);
  ...
  try {
    ...
    const pideListadoMasivo = /(todos|todas|lista|listado|base de datos|exportar|descargar)/i.test(preguntaPriv);
    const pideContactoSensible = /(telefono|telefonos|celular|celulares|contacto|contactos|numero|numeros|whatsapp)/i.test(preguntaPriv);
    const objetivoFamilias = /(padre|padres|madre|madres|tutor|tutores|apoderado|apoderados|familia|familias)/i.test(preguntaPriv);

    if (pideListadoMasivo && pideContactoSensible && objetivoFamilias) {
      const tiempo = Date.now() - inicio;
      return {
        respuesta: 'Lo siento, no puedo compartir teléfonos ni contactos de padres/tutores, ya que es información sensible y confidencial. Si necesita comunicarse con una familia en particular, por favor gestione la solicitud por Secretaría con la autorización correspondiente.',
  ...
  try {
    const aclaracionPagos = detectarAclaracionDetallesPagos(pregunta, historialConversacion);
    if (aclaracionPagos) {
      console.log('🔁 [ejecutarAgente] Aclaración detectada para detalle de cuotas vencidas. Reescribiendo pregunta para continuar el flujo.');
      pregunta = aclaracionPagos.preguntaReescrita;
      clasificacionForzada = aclaracionPagos.clasificacionForzada;
    }

    const aclaracion = detectarAclaracionInscripcion(pregunta, historialConversacion);
    if (!clasificacionForzada && aclaracion) {
      console.log(`🔁 [ejecutarAgente] Aclaración detectada: "${aclaracion.seleccion}". Reescribiendo pregunta para continuar el flujo.`);
  ...
  if (analisisPreguntas.esMultiple) {
    ...
    try {
      ...
      const preguntasClasificadas = clasificarPreguntasMultiples(analisisPreguntas.preguntas, infoRemitente);
      ...
      if (infoRemitente) {
        try {
          const sugerencias = await generarSugerenciasProactivas(infoRemitente, pool);
          if (sugerencias) {
      ...
      console.error(`❌ [ejecutarAgente] Error procesando múltiples preguntas:`, error);
  ...
  if (historialConversacion.length === 0) {
    ...
    const esConsultaDinamicaPagos =
      /(pagar|pagos?|debo|deuda|pendiente|cuota|mensualidad|saldo)\b/i.test(pregunta) &&
      /(febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)/i.test(pregunta);

    const preguntaHash = crypto.createHash('md5').update(pregunta.toLowerCase().trim()).digest('hex');
  ...
  console.log(`🔍 [ejecutarAgente] Info remitente disponible: ${infoRemitente ? 'Sí' : 'No'}`);
  if (infoRemitente) {
  ...
  const clasificacion = clasificacionForzada || clasificarConsulta(pregunta, infoRemitente);
  console.log(`🔍 [ejecutarAgente] Clasificación resultante:`, clasificacion);
  console.log(`🔍 Clasificación: ${clasificacion.tipo} (${clasificacion.herramienta}) - Confianza: ${clasificacion.confianza}`);

  // Analizar sentimiento ANTES de procesar
  const analisisSentimiento = analizarSentimiento(pregunta, historialConversacion);
  ...
  if (infoUsuario) {
    ...
    } else if (infoUsuario.rol === 'Administrador' || infoUsuario.rol === 'Director') {
  ...
  try {
    if (clasificacion.herramienta === 'fecha_hora') {
      herramienta = new HerramientaFechaHora();
      respuesta = await herramienta.ejecutar(pregunta, { historial: contextoHistorial });
    } else if (clasificacion.herramienta === 'ayuda') {
      // Nueva herramienta de ayuda y bienvenida
      herramienta = new HerramientaAyuda();
      respuesta = await herramienta.ejecutar(pregunta, infoRemitente, contextoHistorial, infoUsuario);
      ...
      if (!infoUsuario || (infoUsuario.rol !== 'Administrador' && infoUsuario.rol !== 'Director')) {
        respuesta = '⚠️ Solo administradores y directores pueden enviar mensajes masivos.';
      } else {
        ...
        if (params.error) {
          ...
          try {
            ...
            if (padres.length === 0) {
            ...
            } else if (!whatsappService || !(await whatsappService.isClientReady())) {
              ...
              (async () => {
                ...
                console.log(`📨 Iniciando envío masivo a ${padres.length} padres...`);

                for (const padre of padres) {
                  try {
                    const telefono = padre.telefono_padre || padre.telefono_madre;
                    if (telefono) {
                      const mensajePersonalizado = personalizarMensaje(mensaje, padre);
                      await whatsappService.enviarMensajeANumero(telefono, mensajePersonalizado);
                      enviados++;
                      console.log(`✅ [${enviados}/${padres.length}] Enviado a ${padre.nombre_padre || padre.nombre_madre}`);
              ...
              respuesta = `📨 **Envío Masivo Iniciado**\n\n` +
      ...
      respuesta = `He detectado que quieres enviar una notificación. Para enviar notificaciones a padres y tutores, por favor usa el endpoint de notificaciones manuales:\n\n` +
    ...
    const longitudRespuesta = determinarLongitudRespuesta(pregunta, clasificacion.tipo);
    respuesta = mejorarFormatoRespuesta(respuesta, clasificacion.tipo, longitudRespuesta, pregunta);
    ...
    if (infoRemitente && !analisisSentimiento.tieneFrustracion && !analisisSentimiento.necesitaEscalamiento) {
      try {
        const sugerencias = await generarSugerenciasProactivas(infoRemitente, pool);
        if (sugerencias) {
    ...
    if (historialConversacion.length === 0 && !analisisSentimiento.tieneFrustracion && herramienta.nombre !== 'base_datos') {
      const preguntaHash = crypto.createHash('md5').update(pregunta.toLowerCase().trim()).digest('hex');
...
function registrarConsulta(pregunta, respuesta, herramienta, clasificacion, tiempoMs, usuarioId = null) {
  const logEntry = {
