...
class NotificacionesService {
  ...
  normalizarNumeroParaWhatsApp(numero) {
    ...
    console.log(`⚠️ Número no válido (longitud: ${normalizado.length}): ${normalizado}`);
    return null;
  }
  ...
  async obtenerTelefonosEstudiante(estudianteId) {
    try {
      ...
      if (contactos.length === 0) {
        console.log(`⚠️ Estudiante ${estudianteId} no tiene contactos verificados en contacto_aviso`);
        return [];
      }

      const estudiante = contactos[0];
      const nombreEstudiante = `${estudiante.nombre_estudiante} ${estudiante.apellido_paterno || ''}`.trim();

      // Transformar a formato esperado
      const telefonos = contactos.map(contacto => {
      ...
      console.log(`✅ Encontrados ${telefonosUnicos.length} contacto(s) verificado(s) para estudiante ${estudianteId}`);
  ...
  async revisarVencimientosYNotificar(diasAnticipacion = 2) {
    ...
    try {
      ...
      if (!this.whatsappService || !(await this.whatsappService.isClientReady())) {
        // Log silenciado
        return { enviadas: 0, errores: 0, mensaje: 'WhatsApp no está conectado' };
      }
      ...
      for (const cuota of cuotas) {
        try {
          ...
          if (telefonos.length === 0) {
            console.log(`⚠️ Estudiante ${cuota.nombre_estudiante} no tiene teléfonos registrados`);
            continue;
          }
          ...
          for (const telefono of telefonos) {
            ...
            try {
              await this.whatsappService.client.sendMessage(telefono.numero, mensajePersonalizado);
              enviadas++;
              console.log(`✅ Notificación enviada a ${telefono.nombre} (${telefono.numero})`);

              // Pequeña pausa para evitar rate limiting
              await new Promise(resolve => setTimeout(resolve, 2000));
            } catch (error) {
              errores++;
              console.error(`❌ Error enviando a ${telefono.numero}:`, error.message);
            }
          }
          ...
          errores++;
          console.error(`❌ Error procesando cuota ${cuota.id}:`, error.message);
        }
      }

      console.log(`✅ Proceso de notificaciones completado: ${enviadas} enviadas, ${errores} errores`);
  ...
  async enviarNotificacionManual(mensaje, filtros = {}) {
    try {
      ...
      const turnoFiltro = filtros.turno != null && String(filtros.turno).trim() !== '' ? String(filtros.turno).trim() : null;
      const tieneFiltrosAcademicos =
        filtros.nivel_id || filtros.curso_id || filtros.bloque_id || turnoFiltro;
      const params = [];
      ...
      if (tieneFiltrosAcademicos) {
        ...
        if (turnoFiltro) {
          const tn = turnoFiltro.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
          query += ` AND (
      ...
      const [testQuery] = await pool.query(`
        ...
        INNER JOIN contacto_aviso ca ON e.id = ca.estudiante_id AND ca.activo = TRUE
      `);
      console.log(`🔍 Total de estudiantes con contactos verificados: ${testQuery[0]?.total || 0}`);
      ...
      try {
        ...
        console.log(`✅ Query ejecutada exitosamente. Estudiantes encontrados: ${estudiantes.length}`);
      } catch (sqlError) {
      ...
      if (estudiantes.length === 0) {
        console.warn('⚠️ No se encontraron estudiantes. Verificando posibles causas...');
        ...
        const [sinFiltros] = await pool.query(`
          ...
          INNER JOIN contacto_aviso ca ON e.id = ca.estudiante_id AND ca.activo = TRUE
        `);
        console.log(`📊 Estudiantes con contacto verificado (sin filtros): ${sinFiltros[0]?.total || 0}`);
        ...
        if (tieneFiltrosAcademicos) {
          const [conInscripciones] = await pool.query(`
            ...
            INNER JOIN contacto_aviso ca ON e.id = ca.estudiante_id AND ca.activo = TRUE
          `);
          console.log(`📊 Estudiantes con contacto verificado e inscripciones: ${conInscripciones[0]?.total || 0}`);
        }
      }
      ...
      for (const estudianteRow of estudiantes) {
        try {
          const telefonos = await this.obtenerTelefonosEstudiante(estudianteRow.id);
          console.log(`📱 Estudiante ${estudianteRow.id}: ${telefonos.length} teléfonos encontrados`);
          ...
          for (const telefono of telefonos) {
            ...
            mensajePersonalizado = mensajePersonalizado
              .replace(/\s*(padres|tutores|padres\s+o\s+tutores)\s+(que|:)?\s*/gi, ' ')
              .replace(/\s+/g, ' ') // Normalizar espacios múltiples
              .trim();

            // Si el mensaje no tiene saludo personalizado, agregarlo
            let mensajeFinal = mensajePersonalizado;
            if (!mensajePersonalizado.toLowerCase().includes('buenos días') &&
              ...
              if (!mensajePersonalizado.toLowerCase().includes('saludos') &&
            ...
            try {
              ...
              await this.whatsappService.client.sendMessage(telefono.numero, mensajeFinal);
              ...
              // Clasificar errores
              if (errorMsg.includes('No LID for user') || errorMsg.includes('not registered')) {
                console.warn(`⚠️ Número ${telefono.numero} no tiene WhatsApp o no está registrado`);
              } else if (errorMsg.includes('getChat')) {
                console.warn(`⚠️ Error de conexión WhatsApp al enviar a ${telefono.numero}, reintentando...`);
                ...
                try {
                  if (await this.whatsappService.isClientReady()) {
                    ...
                    console.log(`✅ Notificación enviada a ${telefono.nombre} (${telefono.numero}) - Reintento exitoso`);
                  }
                } catch (retryError) {
                  console.error(`❌ Error en reintento para ${telefono.numero}:`, retryError.message);
                }
              } else {
                console.error(`❌ Error enviando a ${telefono.numero}:`, errorMsg);
              }
            }
          }
        } catch (error) {
          errores++;
          console.error(`❌ Error procesando estudiante ${estudianteRow.id}:`, error.message);
        }
      }
      ...
      throw new Error(`Error al enviar notificación: ${error.message || 'Error desconocido'}`);
    }
  }
  ...
  generarMensajePersonalizado(textoAdmin, fecha = null) {
    ...
    if (textoAdmin.includes('{nombre}') || textoAdmin.includes('{estudiante}')) {
      return textoAdmin;
    }
